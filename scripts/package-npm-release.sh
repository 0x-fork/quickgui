...
arm64_binary=packages/native/lib/darwin-arm64/libquickgui_host.dylib
x64_binary=packages/native/lib/darwin-x64/libquickgui_host.dylib
...
for package_name in native cli; do
...
for archive in "$native_archive" "$cli_archive"; do
  if [[ ! -f $archive ]]; then
...
for entry in \
  package/lib/darwin-arm64/libquickgui_host.dylib \
  package/lib/darwin-x64/libquickgui_host.dylib
do
...
native_binary_count=$(grep -Ec 'libquickgui_host\.dylib$' <<< "$native_entries" || true)
if [[ $native_binary_count != 2 ]]; then
  echo "package-npm-release: native archive must contain exactly two host shared libraries" >&2
  exit 1
fi
...
printf '%s\n' \
  "QUICKGUI_NPM_PACKAGE_RESULT {\"version\":\"$release_version\",\"native\":\"$(basename "$native_archive")\",\"cli\":\"$(basename "$cli_archive")\",\"checksums\":\"$(basename "$checksum_file")\",\"passed\":true}"
