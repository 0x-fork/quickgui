...
const goOptions = new Set([...read("go/ui/options_generated.go").matchAll(/^func (\w+)\(/gm)].map((match) => match[1]));
...
for (const helper of [...helpers.values()].sort((a, b) => a.name.localeCompare(b.name))) {
  ...
  const options = helper.ops.map(([field, value]) => `${goName(field)}(${literal(value, "go")})`).join(", ");
  if (!goOptions.has(name) && name !== "Flex") go += `// ${name} matches Rust's ${helper.name} helper.\nfunc ${name}(${parameters}) StyleOption {\nreturn func(style *Style) {\nfor _, option := range []StyleOption{${options}} { option.applyStyle(style) }\n}\n}\n\n`;
  if (name !== "Flex" && name !== "FlexWrap") go += `// ${name} matches Rust's ${helper.name} helper.\nfunc (element *Element) ${name}(${parameters}) *Element { return element.configureStyles([]string{${helper.ops.map(([field]) => JSON.stringify(goName(field))).join(", ")}}, ${options}) }\n\n`;
