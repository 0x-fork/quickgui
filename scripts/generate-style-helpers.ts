...
const booleanHelpers = [...helpers.values()].filter((helper) => helper.parameters.length === 0);
function generateTypeScriptHelpers() {
  const sorted = [...booleanHelpers].sort((a, b) => a.name.localeCompare(b.name));
  const solid = read("packages/solid/src/index.ts");
