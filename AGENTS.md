...
- Go components and UI construction callbacks use `func()` and declare their children implicitly. Node-returning component factories are unsupported. Native constructors still return builders for fluent modifiers and node references. Plain value props and direct reactive expressions are compiled into retained bindings; no annotations are required.
