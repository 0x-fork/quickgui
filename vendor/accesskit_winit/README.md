...
This is QuickGUI's temporary AccessKit adapter fork. It is source-compatible with
`accesskit_winit` 0.33.2 but links against `quickgui-winit`, keeping accessibility and the
window runtime in one Winit type universe. The public Rust library name remains
`accesskit_winit`.
