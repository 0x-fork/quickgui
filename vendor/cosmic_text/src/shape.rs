...
use crate::{
    ...
    Ellipsize, EllipsizeHeightLimit, Family, FamilyOwned, Font, FontSystem, GlyphDecorationData,
...
fn default_families<'a>(attrs: &'a Attrs<'_>) -> SmallVec<[Family<'a>; 9]> {
    let mut families = SmallVec::new();
    families.push(attrs.family);
    if let Some(fallbacks) = attrs.font_fallbacks.as_deref() {
        families.extend(
            fallbacks
                .iter()
                .map(FamilyOwned::as_family)
                .filter(|family| *family != attrs.family),
        );
    }
    families
}
