...
pub struct Attrs<'a> {
    ...
    pub family: Family<'a>,
    /// Ordered family preferences tried after [`Self::family`] and before platform fallbacks.
    pub font_fallbacks: Option<Arc<[FamilyOwned]>>,
    pub stretch: Stretch,
    pub style: Style,
    pub weight: Weight,
    pub metadata: usize,
...
impl<'a> Attrs<'a> {
    ...
    /// Set ordered family preferences tried after the primary family.
    pub fn font_fallbacks(mut self, font_fallbacks: Arc<[FamilyOwned]>) -> Self {
        self.font_fallbacks = (!font_fallbacks.is_empty()).then_some(font_fallbacks);
        self
    }
...
impl AttrsOwned {
    pub fn new(attrs: &Attrs) -> Self {
        Self {
            ...
            font_fallbacks: attrs.font_fallbacks.clone(),
            stretch: attrs.stretch,
            style: attrs.style,
            weight: attrs.weight,
...
mod tests {
    ...
    #[test]
    fn owned_attributes_preserve_fallbacks_and_features_in_shape_identity() {
        let fallbacks: Arc<[FamilyOwned]> = Arc::from([
            FamilyOwned::new(Family::Name("Inter")),
            FamilyOwned::new(Family::Name("Noto Sans Hebrew")),
        ]);
        let mut features = FontFeatures::new();
        features.disable(FeatureTag::CONTEXTUAL_ALTERNATES);
        let attrs = Attrs::new()
            .family(Family::Name("Missing Primary"))
            .font_fallbacks(Arc::clone(&fallbacks))
            .font_features(features);
        let owned = AttrsOwned::new(&attrs);

        assert_eq!(owned.as_attrs(), attrs);
        assert!(!attrs.compatible(&Attrs::new().family(Family::Name("Missing Primary"))));

        let different_fallbacks = Attrs::new()
            .family(Family::Name("Missing Primary"))
            .font_fallbacks(Arc::from([FamilyOwned::new(Family::Name("Inter"))]));
        assert!(!attrs.compatible(&different_fallbacks));
    }
}
