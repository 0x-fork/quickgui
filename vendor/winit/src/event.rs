...
pub enum WindowEvent {
    ...
    /// A macOS mouse button event with AppKit's exact multi-click count.
    ///
    /// Winit normally buffers `MouseInput` after returning from `sendEvent:`, at which point
    /// `NSApplication.currentEvent` no longer identifies the matching native event. The
    /// versioned QuickGUI support crate retains the bounded scalar while AppKit is synchronous.
    #[cfg(target_os = "macos")]
    MouseInputWithClickCount {
        device_id: DeviceId,
        state: ElementState,
        button: MouseButton,
        click_count: u16,
