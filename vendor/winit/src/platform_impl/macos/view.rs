...
use crate::event::{
    DeviceEvent, ElementState, Ime, KeyEvent, Modifiers, MouseButton, MouseScrollDelta, Touch,
...
declare_class!(
    ...
    unsafe impl WinitView {
        ...
        fn draw_rect(&self, _rect: NSRect) {
            ...
            if let Some(window) = self.ivars()._ns_window.load() {
                self.ivars().app_delegate.handle_redraw(window_id(&window));
            }
    ...
    unsafe impl WinitView {
        ...
        fn pressure_change_with_event(&self, event: &NSEvent) {
            ...
            // Pressure is targeted at the event's precise cursor location, just like magnify,
            // smart-magnify, and rotation above. Queue motion first so consumers do not receive a
            // stale position when the pointer moved between pressure updates.
            self.mouse_motion(event);
...
impl WinitView {
    ...
    fn queue_event(&self, event: WindowEvent) {
        self.ivars()
            .app_delegate
            .maybe_queue_window_event(window_id(&self.window()), event);
    }
    ...
    fn queue_touches(&self, event: &NSEvent, native_phase: NSTouchPhase, phase: TouchPhase) {
        ...
        for touch in touches.iter() {
            ...
            let location = LogicalPosition::new(point.x, point.y).to_physical(scale_factor);
    ...
    fn mouse_click(&self, event: &NSEvent, button_state: ElementState) {
        ...
        let click_count = unsafe { event.clickCount() }.clamp(1, u16::MAX as isize) as u16;

        self.update_modifiers(event, false);

        self.queue_event(WindowEvent::MouseInputWithClickCount {
            device_id: DEVICE_ID,
            state: button_state,
            button,
            click_count,
...
fn mouse_button(event: &NSEvent) -> MouseButton {
    match unsafe { event.r#type() } {
        NSEventType::LeftMouseDown | NSEventType::LeftMouseUp | NSEventType::LeftMouseDragged => {
            return MouseButton::Left
        }
        NSEventType::RightMouseDown
        | NSEventType::RightMouseUp
        | NSEventType::RightMouseDragged => return MouseButton::Right,
        _ => {}
    }
