...
This is QuickGUI's temporary Winit 0.30.13 support fork. It backports the upstream native
`NSPanel` allocation API needed by anchored popups and adds macOS raw-touch delivery while
AccessKit remains on Winit 0.30. The public Rust library name remains `winit`, so downstream code
uses the upstream API.

The original Winit README follows.
