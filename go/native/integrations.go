...
type UpdateClientOptions struct {
	...
	MaximumDownloadBytes uint64 `json:"maximumDownloadBytes,omitempty"`
}
...
type UpdateInstallOptions struct {
	TargetExecutable   string   `json:"targetExecutable,omitempty"`
	RetainBackup       *bool    `json:"retainBackup,omitempty"`
	WindowsMode        string   `json:"windowsMode,omitempty"`
	InstallerArguments []string `json:"installerArguments,omitempty"`
}
...
type InstalledUpdate struct {
	...
	InstallerProcessID      uint32 `json:"installerProcessId,omitempty"`
	RequiresApplicationExit bool   `json:"requiresApplicationExit"`
	RelaunchRecommended     bool   `json:"relaunchRecommended"`
}
...
// Updater checks, downloads, verifies, and installs through the native updater.
// No checks or downloads happen until explicitly requested by the application.
var Updater updaterAPI

func (updaterAPI) DefaultTarget() (string, error) {
	return callJSON[string]("default-update-target", nil)
}

func (updaterAPI) Check(endpoint string, options UpdateClientOptions, done func(*AvailableUpdate, error)) {
	invokeJSON("check-for-update", map[string]any{"endpoint": endpoint, "options": options}, done)
}

func (updaterAPI) Stage(update AvailableUpdate, destinationDirectory string, options UpdateClientOptions, progress func(UpdateProgress), done func(string, error)) {
	var listener func(string)
	if progress != nil {
		decode := decodeServiceReply("update-progress", func(value UpdateProgress, err error) {
			if err == nil {
				progress(value)
			}
		})
		listener = func(raw string) { decode(raw, nil) }
	}
	invokeWithProgress("stage-update", map[string]any{
		"update": update, "destinationDirectory": destinationDirectory,
		"options": options, "progress": progress != nil,
	}, listener, decodeServiceReply("stage-update", done))
}

func (updaterAPI) Verify(path, signature string, options UpdateClientOptions, done func(error)) {
	invokeVoid("verify-update", map[string]any{"path": path, "signature": signature, "options": options}, done)
}

func (updaterAPI) Install(update AvailableUpdate, artifact string, options UpdateClientOptions, installOptions UpdateInstallOptions, done func(InstalledUpdate, error)) {
	invokeJSON("install-update", map[string]any{
		"update": update, "artifact": artifact, "options": options, "installOptions": installOptions,
	}, done)
}
