...
// [github.com/egoist/quickgui/go/native.Run] owns.
package reactive
...
func identical[T any](a, b T) bool {
...
func link(listener *Computation, source linkable) {
	src := source.asSource()
	// Repeated reads during one computation share a subscription.
	for _, existing := range listener.Sources {
		if existing == source {
			return
		}
	}
...
func completeUpdates() {
	for turns := 0; ; turns++ {
		if turns == 10000 {
			panic("QuickGUI reactive update cycle exceeded 10000 turns")
		}
		if len(Updates) > 0 {
			batch := Updates
			Updates = nil
...
func updateComputation(node *Computation) {
	if node.Running {
		panic("QuickGUI reactive computation cycle")
	}
	if node.Disposed {
		return
	}
	node.Running = true
	defer func() { node.Running = false }()
	cleanNode(node)
...
func detachOwner(owner *Owner) {
	...
	for i, child := range parent.Owned {
		if child == owner {
			parent.Owned = slices.Delete(parent.Owned, i, i+1)
			return
		}
	}
}
