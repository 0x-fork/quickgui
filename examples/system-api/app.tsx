...
import { createSignal, onCleanup } from "@quickgui/ui";
...
function SystemApiExample() {
  ...
  onCleanup(
    app.onSecondInstance((event) => {
      window.show();
      window.focus();
      setStatus(`A second launch forwarded ${event.argv.length} argument(s).`);
    }),
  );
  onCleanup(app.onOpenUrls((event) => setStatus(`Deep link: ${event.urls.join(", ")}`)));
  onCleanup(
    PowerMonitor.onEvent((event) => {
      if (event.type === "suspend") setStatus("The system is suspending.");
      else if (event.type === "resume") setStatus("The system resumed.");
      else if (event.type === "lock-screen") setStatus("The session was locked.");
      else if (event.type === "unlock-screen") setStatus("The session was unlocked.");
    }),
  );
  onCleanup(
    app.onNotificationResponse((event) => {
      const reply = event.reply;
      const actionId = event.actionId;
      const response = reply !== undefined ? `: ${reply}` : actionId !== undefined ? ` via ${actionId}` : "";
      setStatus(`Notification ${event.tag} activated${response}.`);
    }),
  );
  onCleanup(
      ...
      >
        ...
        <View style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
          {action("App environment", async () => {
            ...
            const system = await app.getSystemInfo();
              ...
              entries: [
                {
                  ...
                  metadata: JSON.stringify({ source: "system-api" }),
                },
                {
                  type: "data",
                  mimeType: "text/html",
          ...
          {action("Permissions", async () => {
            const kinds: PermissionKind[] = ["camera", "microphone", "screen-recording", "accessibility"];
            return kinds.map((permission) => `${permission}: ${Permissions.status(permission)}`).join(" · ");
          })}
          ...
          {action("Tray icon", async () => {
            ...
            tray = await Tray.create({ icon: makeTrayIcon(), tooltip: appName, menu });
            tray.onEvent((event) => {
              if (event.kind === "click") setStatus("Tray icon clicked.");
            });
