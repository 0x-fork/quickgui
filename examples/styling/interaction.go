					...
					children = append(children, ui.View().Child(
						func() *native.Node {
							...
							for row := range 6 {
								...
								).Display("flex").AlignItems("center").Height(30).FlexShrink(0).PaddingLeft(12).Node)
							}
							return ui.Fragment(children)
						},
					).Display("flex").FlexDirection("column").FlexShrink(0).Node)
				}
				return ui.Fragment(children)
			},
			...
			func() *native.Node {
				...
				for index, tint := range tints {
					children = append(children, ui.View().Children(
						...
						centered,
					).Width(200).Height(110).FlexShrink(0).ScrollSnapAlign("start").ScrollSnapStop("always").BorderRadius(12).BackgroundColor(tint).Node)
				}
				return ui.Fragment(children)
			},
		).Height(130).OverflowX("scroll").ScrollSnapType("x mandatory").Display("flex").Gap(12).Padding(6).BorderRadius(10).BorderWidth(1).BorderColor(panelBorder).Node,
			ui.Text(
