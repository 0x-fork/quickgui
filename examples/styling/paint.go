...
func Gradients() *ui.Element {
	...
	return Panel("Gradients", func() *native.Node {
		return ui.Fragment([]*native.Node{swatch("linear-gradient", tile, ui.Style().Background("linear-gradient(135deg, #1d4ed8, #38bdf8 60%, #a855f7)")).Node,
			swatch("radial-gradient", tile, ui.Style().Background("radial-gradient(circle closest-side at 30% 40%, #f8fafc, #0f172a)")).Node,
...
func BordersAndOutlines() *ui.Element {
	return Panel("Corners, borders, and outlines", func() *native.Node {
		return ui.Fragment([]*native.Node{ui.View().Height(56).BorderRadius("22px 6px 22px 6px").BackgroundColor("#1b2434").BorderWidth(1).BorderColor("#38bdf8").Node,
			ui.View().Height(56).BorderRadius(12).BorderWidth(2).BorderColor("#f97316").BorderStyle("dashed").Node,
			ui.View().Height(56).BorderRadius(12).BorderWidth(2).BorderColor("#22c55e").BorderStyle("dotted").Node,
			ui.View().Height(56).Margin(6).BorderRadius(12).BackgroundColor("#1b2434").Outline("2px solid #a855f7").OutlineOffset(4).Node,
			ui.View().Height(56).Margin(6).BorderTopLeftRadius(28).BorderBottomRightRadius(28).BackgroundColor("#1b2434").Outline("2px dashed #38bdf8").OutlineOffset(3).Node})
	})
}
	...
	return Panel("Filters and backdrop", func() *native.Node {
		...
		children = append(children, ui.View(
			...
			swatch("backdropFilter", ui.Style().
				...
				FontWeight(700)),
		).Position("relative").Height(84).BorderRadius(12).Padding(12).Background("conic-gradient(from 30deg, #1d4ed8, #f97316, #1d4ed8)").Node)
...
func Transforms() *ui.Element {
	...
	return Panel("Transforms and blending", func() *native.Node {
		return ui.Fragment([]*native.Node{swatch("rotate(-3deg)", card, ui.Style().Transform("rotate(-3deg)")).Node,
			...
			ui.View(
				func() *native.Node {
					...
					for _, tile := range []struct{ color, blend string }{
						...
						children = append(children, ui.View().Flex(1).BorderRadius(10).BackgroundColor(tile.color).MixBlendMode(tile.blend).Node)
					}
					return ui.Fragment(children)
				},
			).Display("flex").Height(84).Gap(10).Padding(10).BorderRadius(12).Background("linear-gradient(90deg, #f97316, #38bdf8)").Node})
	})
}
