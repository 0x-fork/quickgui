...
func BordersAndOutlines() {
	Panel("Corners, borders, and outlines", func() {
		ui.View().Height(56).BorderRadius("22px 6px 22px 6px").BackgroundColor("#1b2434").BorderWidth(1).BorderColor("#38bdf8")
		ui.View().Height(56).BorderRadius(12).BorderWidth(2).BorderColor("#f97316").BorderStyle("dashed")
		ui.View().Height(56).BorderRadius(12).BorderWidth(2).BorderColor("#22c55e").BorderStyle("dotted")
		ui.View().Height(56).Margin(6).BorderRadius(12).BackgroundColor("#1b2434").Outline("2px solid #a855f7").OutlineOffset(4)
		ui.View().Height(56).Margin(6).BorderTopLeftRadius(28).BorderBottomRightRadius(28).BackgroundColor("#1b2434").Outline("2px dashed #38bdf8").OutlineOffset(3)
	})
}
...
func Filters() {
	...
	Panel("Filters and backdrop", func() {
		...
		).Position("relative").Height(84).BorderRadius(12).Padding(12).Background("conic-gradient(from 30deg, #1d4ed8, #f97316, #1d4ed8)")
	})
}
		...
		ui.View(
			func() {
				for _, tile := range []struct{ color, blend string }{
					...
					ui.View().Flex(1).BorderRadius(10).BackgroundColor(tile.color).MixBlendMode(tile.blend)
				}
			},
		).Display("flex").Height(84).Gap(10).Padding(10).BorderRadius(12).Background("linear-gradient(90deg, #f97316, #38bdf8)")
	})
}
