...
func sectionHeader(m *model, label, trailing string, action func()) *ui.Element {
	return ui.View().Child(
		func() *native.Node {
			...
			if action != nil {
				children_ = append(children_, iconButton(m, "Add space", "plus", 20, action, ui.Style().MarginLeft(5)).Node)
			}
...
func spaceRow(m *model, s space) *ui.Element {
	...
	return ui.View().Children(
		...
		ui.Show(
			...
			func() *ui.Element {
				return iconButton(m, "Remove "+s.Name+" space", "close", 20, func() { m.removeSpace(s) }, ui.Style().
					Position("absolute").
					Top(14).
					Right(5))
			},
