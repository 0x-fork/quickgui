...
func sidebar(m *model) *native.Node {
	return ui.Fragment([]*native.Node{ui.View().Children(
		...
		ui.View().Children(
			...
			ui.View().Flex(1),
			ui.Button().Style(m.iconStyle(24)).Child(dynamicIcon(func() string { return choose(m.Appearance.Read() == "dark", "sun", "moon") }, 14, m.color(func(t theme) string { return t.TextTertiary }))).AriaLabel("Toggle light or dark appearance").FocusOnPointer(ptr(false)).OnClick(m.toggleTheme),
		...
		sidebarSection(m.SectionRatio.Read, func() *native.Node {
			return ui.Fragment([]*native.Node{sectionHeader(m, "Spaces", "", func() { m.addSpace() }).Node,
				sidebarList(func() *native.Node {
					return ui.For(
						m.Spaces.Read,
						func(s space, _ func() int) *ui.Element { return spaceRow(m, s) },
		...
		sidebarSection(func() float64 { return 1 - m.SectionRatio.Read() }, func() *native.Node {
			return ui.Fragment([]*native.Node{sectionHeader(m, "Agents", "grouped", nil).Node,
				sidebarList(func() *native.Node {
					return ui.For(
						m.visibleAgents,
	...
	return ui.View().Child(
		func() *native.Node {
			...
			children_ = append(children_, ui.Text(
				label,
			).TextColor(m.color(func(t theme) string { return t.TextTertiary })).FontSize(12).FontWeight(650).Node)
			children_ = append(children_, ui.View().Flex(1).Node)
			if trailing != "" {
				children_ = append(children_, ui.Text(
					trailing,
				).TextColor(m.color(func(t theme) string { return t.TextGhost })).FontSize(10.5).Node)
			}
			if action != nil {
...
func spaceRow(m *model, s space) *ui.Element {
	...
	return ui.View().Children(
		...
		ui.Button().Style(m.rowStyle(selected)).Children(
			statusGlyph(m, func() string { return aggregateStatus(agents()) }, true),
			twoLineRow(func() *native.Node {
				return ui.Fragment([]*native.Node{rowLine(func() *native.Node {
					return ui.Fragment([]*native.Node{rowTitle(m, s.Name).Node,
						ui.View().Flex(1).Node,
						ui.Show(
							len(agents()) > 0,
							func() *ui.Element {
		...
		ui.Show(
			...
			func() *ui.Element {
				return iconButton(m, "Remove "+s.Name+" space", "close", 20, func() { m.removeSpace(s) }).Style(ui.Style().
					Position("absolute").
					Top(14).
					Right(5))
			},
...
func agentRow(m *model, p *pane) *ui.Element {
	return ui.Button().Style(m.rowStyle(func() bool { return m.ActivePaneID.Read() == p.ID })).Children(
		statusGlyph(m, func() string { return p.Status.Read().AgentStatus }, false),
		twoLineRow(func() *native.Node {
			return ui.Fragment([]*native.Node{rowLine(func() *native.Node {
				return ui.Fragment([]*native.Node{rowTitle(m, func() string {
				...
				rowLine(func() *native.Node {
					return ui.Fragment([]*native.Node{rowMeta(m, func() string { return agentLabel(p.Status.Read().Agent) }).Node,
						ui.View().Flex(1).Node,
						ui.Text(
							...
							).FontSize(11.5).LineHeight(15).Node})
				}).Node})
		}),
