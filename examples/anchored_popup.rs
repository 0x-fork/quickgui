use quickgui::{
    AnchorPlacement, AnchoredPopover, App, AppConfig, Color, Element, IntoElement, PopupMenu,
    PopupMenuItem, PopupMenuItemKind, PopupMenuItemState, View, ViewContext, WindowHandle, button,
    div, popup_menu_key_bindings, text,
};

fn main() -> Result<(), quickgui::AppError> {
    App::new(PopupLauncher::default())
        .bind_keys(popup_menu_key_bindings())
        .config(
            AppConfig::new("QuickGUI — Anchored native popup")
                .size(720.0, 480.0)
...
impl View for PopupLauncher {
    fn render(&mut self, cx: &mut ViewContext<'_, Self>) -> impl IntoElement {
        cx.on_any_child_window_closed(|this, closed, cx| {
            if this.popup == Some(closed) {
                this.popup = None;
                cx.invalidate();
            }
        });
        let command = cx.action_listener("popup-launcher", |this, command: &DemoCommand, cx| {
        ...
        let open = cx.listener("open-native-popup", |this, cx| {
            if let Some(previous) = this.popup.take() {
                cx.close_window_handle(previous);
            }
            this.opened = this.opened.saturating_add(1);
            this.popup = Some(
                AnchoredPopover::new(244.0, 190.0)
                    .gap(6.0)
                    .open(
                        cx,
                        "open-native-popup",
                        "Actions",
                        PopupMenuView::new(demo_menu(this.sidebar)),
                    )
                    .expect("the mounted trigger can open a native popup"),
            );
            cx.invalidate();
        });

        div()
            .focus_scope(cx.focus_handle("popup-launcher"))
            .on_action(command)
            .size_full()
            .relative()
            ...
            .child(
                button()
                    ...
                    .child(text("Open actions").font_medium())
                    .on_click(open),
            )
            .child(
                div()
                    ...
                    .child(
                        text("The popup resolves this button's retained bounds by ID—there is no copied trigger rectangle. It can cross the parent edge, while native work-area constraints choose the better side near a display edge. Escape or an outside press dismisses this grabbing menu; passive anchored popups remain attached while the parent moves.")
                            .wrap()
                            .text_sm()
    ...
    fn render(&mut self, cx: &mut ViewContext<'_, Self>) -> impl IntoElement {
        ...
        self.menu.element_with_submenus(
            ...
            |view, anchor, menu, cx| {
                ...
                view.submenu = Some(
                    AnchoredPopover::new(220.0, menu_height(&menu))
                        .placement(AnchorPlacement::RightStart)
                        .gap(2.0)
                        .open(cx, anchor, "More actions", PopupMenuView::new(menu))
                        .expect("a mounted menu item can anchor a nested popup"),
                );
            },
        )
    }
...
fn demo_menu(sidebar: bool) -> PopupMenu {
    let more = PopupMenu::new([
        PopupMenuItem::action("settings", "Settings", DemoCommand::Settings).shortcut("⌘,"),
        PopupMenuItem::action("about", "About QuickGUI", DemoCommand::About),
    ])
    .expect("static submenu is valid");
    PopupMenu::new([
        PopupMenuItem::group_label("File"),
        PopupMenuItem::action("new", "New window", DemoCommand::NewWindow).shortcut("⌘N"),
        PopupMenuItem::action("open", "Open…", DemoCommand::Open).shortcut("⌘O"),
        PopupMenuItem::separator(),
        PopupMenuItem::checkbox_with("sidebar", "Show sidebar", sidebar, DemoCommand::SetSidebar),
        PopupMenuItem::submenu("more", "More", more),
    ])
    .expect("static popup menu is valid")
}
...
fn menu_item(item: &PopupMenuItem, state: PopupMenuItemState) -> Element {
    match item.kind() {
        ...
        PopupMenuItemKind::GroupLabel => div().h(22.0).px_3().flex_row().items_center().child(
...
fn menu_height(menu: &PopupMenu) -> f32 {
    16.0 + menu
        .items()
        .iter()
        .map(|item| match item.kind() {
            PopupMenuItemKind::Separator => 9.0,
            PopupMenuItemKind::GroupLabel => 22.0,
            _ => 34.0,
        })
        .sum::<f32>()
}
