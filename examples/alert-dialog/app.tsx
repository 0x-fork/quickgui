...
function AlertDialogExample() {
  ...
  async function showSaveDialog() {
    await present(async () => {
      const response = await Dialog.showAlertDialog({
        level: "warning",
      ...
      return `Save dialog result: ${["Save", "Don't Save", "Cancel"][response] ?? String(response)}`;
    });
  }
        ...
        >
          <View style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <Text style={{ color: "#9aa6b7", fontSize: 14, lineHeight: 21 }}>
              Call Dialog.showAlertDialog from an event handler, then await the selected button.
            </Text>
          </View>
