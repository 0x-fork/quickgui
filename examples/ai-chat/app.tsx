...
import { streamCompletion } from "./completion.ts";
import { app, type NativeNode, Window } from "@quickgui/native";
import { Button, createRenderer, Input, Markdown, Text, View, VirtualList } from "@quickgui/ui";
import { SystemPopover } from "@quickgui/ui/popover";
import { For, createSignal, flush } from "@quickgui/ui";
import {
...
const [activeRequest, setActiveRequest] = createSignal<ActiveChatRequest | undefined>(undefined);
...
function commitConversations(update: (current: Conversation[]) => Conversation[]): void {
  setConversations(update(nativeUntrack(conversations)));
...
function retainActiveDraft(): void {
  const currentConversationId = activeConversationId();
  const currentDraft = draft();
  setConversations(((current: ReturnType<typeof conversations>) =>
    current.map((conversation) =>
      conversation.id === currentConversationId && conversation.draft !== currentDraft
        ? { ...conversation, draft: currentDraft }
        : conversation,
    ))(nativeUntrack(conversations)),
  );
}
...
function queueHistoryWrite(): void {
  ...
  if (!snapshot) return;
  historyWrite = writeHistoryAfter(historyWrite, snapshot);
}

async function writeHistoryAfter(previous: Promise<void>, snapshot: ConversationHistory): Promise<void> {
  try { await previous; } catch {}
  try { await saveConversationHistory(snapshot, historyPath); }
...
/** The fields of a message that change after it is created. */
...
// Spelled out instead of `{ ...message, ...update }`: scriptc copies every field of a spread
// `Partial` and narrows it to the target type, so a field the patch leaves out throws at runtime.
function patchMessage(message: ChatMessage, update: MessagePatch): ChatMessage {
  ...
  if (update.failed !== undefined) next.failed = update.failed;
  return next;
}
      ...
      conversation.id === conversationId
        ? {
            ...
            messages: conversation.messages.map((message) =>
              message.id === messageId ? patchMessage(message, update) : message,
            ),
            updatedAt: Date.now(),
          }
        : conversation,
...
async function sendMessage(submittedValue?: string): Promise<void> {
  ...
  try {
    await streamCompletion(key, modelMessages, controller.signal, (chunk) => {
      if (committer.append(chunk)) return true;
    ...
    >
      ...
      >
        ...
        <Button
          ...
          onClick={() => {
            setRevealed(((current: ReturnType<typeof revealed>) => !current)(nativeUntrack(revealed)));
            flush();
            keyInput?.focus();
          }}
          disabled={saving()}
...
function HistorySidebar() {
  ...
  return (
    ...
    >
      ...
      >
        <KeyedFor each={conversations()} key={(conversation) => conversation.id}>
          {(conversation) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <KeyedFor each={messages()} key={(message) => message.id}>
...
openMainWindow();
app.onBeforeQuit(() => { void persistAndQuit(); });
async function persistAndQuit(): Promise<void> {
