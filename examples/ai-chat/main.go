...
func buttonStyle() ui.StyleBuilder {
	return ui.Style().Display("flex").AlignItems("center").JustifyContent("center").Height(34).FlexShrink(0).PaddingLeft(12).PaddingRight(12).BorderRadius(7).BackgroundColor("#253855").TextColor("#e2e8f0").UserSelect("none").AppRegion("no-drag").Cursor("default").FontSize(13).Hover(func(s ui.StyleBuilder) ui.StyleBuilder {
...
func inputStyle() ui.StyleBuilder {
	return ui.Style().Height(38).Width("100%").PaddingLeft(12).PaddingRight(12).BackgroundColor("#0b1020").TextColor("#e2e8f0").BorderRadius(7).BorderWidth(1).BorderColor("#334155").FontSize(14).AppRegion("no-drag")
}
func (controller *chatController) view() *ui.Element {
	return ui.View().Children(
		...
		ui.View().Children(
			controller.toolbar(),
			ui.VirtualList().Child(ui.KeyedFor(
...
func (controller *chatController) sidebar() *ui.Element {
	return ui.View().Children(
		...
		ui.Button().Child("New chat").Style(buttonStyle()).Disabled(controller.busy.Read).OnClick(controller.newConversation),
		ui.View().Child(ui.KeyedFor(
			func() []Conversation {
			...
			func(conversation func() Conversation, _ func() int) *ui.Element {
				return ui.Button().Children(
					ui.Text(conversation().Title).Width("100%").FontWeight(600).WhiteSpace("nowrap").Overflow("hidden").TextOverflow("ellipsis"),
					ui.Text(time.UnixMilli(conversation().UpdatedAt).Format("Jan 2, 15:04")).FontSize(11).TextColor("#94a3b8"),
				...
				).Disabled(controller.busy.Read).OnClick(func() {
					controller.selectConversation(conversation().ID)
				})
			},
			nil,
		)).Display("flex").FlexDirection("column").Flex(1).MinHeight(0).OverflowY("scroll").Gap(6),
		ui.Text("Conversations and drafts are saved locally.").FontSize(11).TextColor("#94a3b8").LineHeight(16),
...
func (controller *chatController) toolbar() *ui.Element {
	return ui.View().Children(
		ui.View().Children(
			ui.Text(controller.current().Title).FontSize(18).FontWeight(700).WhiteSpace("nowrap").TextOverflow("ellipsis").Overflow("hidden"),
			ui.Text("DeepSeek · native streaming Markdown").FontSize(12).TextColor("#94a3b8"),
		...
		func() *native.Node {
			...
			return systemPopover1.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{systemPopover1.Trigger(ui.PopoverTriggerProps{PartProps: ui.PartProps{Style: buttonStyle()}}).Children(func() string {
					if controller.key.Read() == "" {
						return "Set API key"
					}
					return "Provider settings"
				}).NativeNode(), systemPopover1.Content(ui.PopoverContentProps{
					Width:          460,
					Height:         310,
					Placement:      "bottom-end",
					Gap:            ptr(8.0),
					ViewportMargin: ptr(12.0),
				}).Children(controller.providerSettings).NativeNode()})
			}).NativeNode()
		}(),
...
func messageCard(message func() ChatMessage) *ui.Element {
	return ui.View().Children(
		...
		ui.Show(
			message().Failed,
			func() *ui.Element {
				return ui.Text("Response interrupted. You can send another message to continue.").FontSize(12).TextColor("#fca5a5")
			},
		),
...
func (controller *chatController) composer() *ui.Element {
	return ui.View().Children(
		...
		ui.View().Children(
			ui.Input().Style(inputStyle()).Flex(1).MinWidth(0).Value(controller.current().Draft).Placeholder("Message DeepSeek…").Disabled(controller.busy.Read).OnInputEvent(func(event *native.Event) {
...
func (controller *chatController) providerSettings() *ui.Element {
	...
	return ui.View().Children(
		...
		ui.Text("Stored in your operating system’s credential store. Your key is never written to conversation history.").FontSize(12).LineHeight(18).TextColor("#94a3b8"),
		ui.View().Children(
			ui.Input().Style(inputStyle()).Flex(1).MinWidth(0).Value(value()).Password(!reveal()).Disabled(controller.credentialBusy.Read).OnInputEvent(func(event *native.Event) {
			...
			ui.Button().Child(func() string {
				...
				return "Reveal"
			}).Style(buttonStyle()).OnClick(func() {
				setReveal(!reveal())
			}),
		...
		ui.View().Flex(1),
		ui.View().Children(
			ui.Button().Child("Remove key").Style(buttonStyle()).TextColor("#fca5a5").Disabled(controller.credentialBusy.Read).OnClick(func() {
			...
			ui.View().Flex(1),
			ui.Button().Child("Cancel").Style(buttonStyle()).Disabled(controller.credentialBusy.Read).OnClick(close),
			ui.Button().Child("Save").Style(buttonStyle()).BackgroundColor("#2563eb").Disabled(controller.credentialBusy.Read).OnClick(func() {
