...
export function parseConversationHistory(value: unknown): ConversationHistory | undefined {
  ...
  for (const candidate of value.conversations.slice(0, MAX_STORED_CONVERSATIONS)) {
...
export async function saveConversationHistory(
  ...
  try {
    ...
    try { await rm(temporaryPath); } catch {}
    throw error;
  }
}
...
function parseConversation(value: unknown): Conversation | undefined {
  ...
  for (const candidate of value.messages.slice(-MAX_STORED_MESSAGES)) {
