...
func FieldDemo() *ui.Element {
	...
	return panel("Field", "A control, its label, description, error, and validity. Validation triggers and debounce are reported by the core.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return field52.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{field52.Label(ui.FieldLabelProps{}).Children("Email address").NativeNode(), field52.Control(fieldInput(email, setEmail, "you@example.com")).NativeNode(), field52.Description(ui.PartProps{}).Children("We only use this address for receipts.").NativeNode(), field52.Error(ui.PartProps{Style: ui.Style().FontSize(12).TextColor(color(func(p palette) string {
					return p.Danger
				}))}).Children("Enter an address containing @").NativeNode(), field52.Validity(ui.FieldValidityProps{}).Children(func() *ui.Element {
...
func FieldsetDemo() *ui.Element {
	...
	return panel("Fieldset", "A semantic group with a legend. A single flag disables every nested field.", func() *native.Node {
		...
		children = append(children, func() *native.Node {
			...
			return fieldset53.Root().Children(func() *native.Node {
				var children []*native.Node
				children = append(children, fieldset53.Legend(ui.PartProps{}).Children("Profile").NativeNode())
				children = append(children, fieldset53.Description(ui.PartProps{}).Children("Your public identity").NativeNode())
				for _, field := range []struct {
					...
					children = append(children, func() *native.Node {
						...
						return field54.Root().Children(func() *native.Node {
							return ui.Fragment([]*native.Node{field54.Label(ui.FieldLabelProps{}).Children(field.label).NativeNode(), field54.Control(fieldInput(field.value, field.set, field.label)).NativeNode()})
						}).NativeNode()
					}())
				}
...
func FormDemo() *ui.Element {
	...
	return panel("Form", "Validation appears on submission. Return in the email field also submits; Reset clears all form state.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return fieldset55.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{fieldset55.Legend(ui.PartProps{}).Children("Sign up").NativeNode(), func() *native.Node {
					...
					return field56.Root().Children(func() *native.Node {
						var children []*native.Node
						children = append(children, field56.Label(ui.FieldLabelProps{}).Children("Email").NativeNode())
						...
						children = append(children, field56.Error(ui.PartProps{Style: ui.Style().FontSize(12).TextColor(color(func(p palette) string {
							return p.Danger
						}))}).Children("An address containing @ is required").NativeNode())
						return ui.Fragment(children)
					}).NativeNode()
				}(), label("Plan").Node, func() *native.Node {
					...
					return radioGroup57.Root().Children(func() *native.Node {
						var children []*native.Node
				...
				}, "I accept the terms"), ui.Show(
					errors() && !terms(),
					func() *ui.Element {
						return ui.Text("The terms must be accepted").FontSize(12).TextColor(color(func(p palette) string {
							return p.Danger
						}))
					},
				), row(func() *native.Node {
					return ui.Fragment([]*native.Node{primary("Submit", submit).Node, button("Reset", func() {
						ui.Batch(func() {
							setEmail("")
							setPlan("pro")
							setTerms(false)
...
func InputDemo() *ui.Element {
	...
	return panel("Input", "Controlled native editors: single-line, password, and multiline. Return in the first field reports submission.", func() *native.Node {
		return ui.Fragment([]*native.Node{input(text, setText, "Type and press Return").Style(ui.Style().Width(300)).OnSubmitEvent(func(*native.Event) {
			setSubmits(submits() + 1)
		}).Node, input(secret, setSecret, "Password").Style(ui.Style().Width(300)).Password(true).Node, input(notes, setNotes, "Notes").Style(ui.Style().Width(420).Height(96)).Multiline(true).Node, note(func() string {
...
func numberControls(field *ui.NumberFieldComponent) *native.Node {
	return field.Group(rowPart()).Child(func() *native.Node {
		var children []*native.Node
		children = append(children, field.Decrement(control()).Child("−").NativeNode())
		s := inputStyle()
		s = s.Width(110)
		children = append(children, field.Input(ui.NumberFieldInputProps{InputPartProps: ui.InputPartProps{PartProps: ui.PartProps{Style: s}}}).NativeNode())
...
func NumberFieldDemo() *ui.Element {
	...
	return panel("Number field", "Parsing, clamping, step snapping, and scrub gestures. Alt uses the small step; Shift uses the large step; Return commits.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return numberField59.Root().Children(func() *native.Node {
				var children []*native.Node
				state := ui.UseNumberFieldState()
				children = append(children, numberField59.ScrubArea(ui.PartProps{Style: ui.Style().Cursor("ew-resize").Height(24)}).Children("Drag here to change quantity").NativeNode())
				children = append(children, numberControls(numberField59))
				children = append(children, numberField59.ScrubAreaCursor(ui.PartProps{}).Children(func() *native.Node {
					return ui.Show(
						state().Scrubbing,
						func() *ui.Element {
			...
			return numberField60.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{label("Read-only").Node, numberControls(numberField60)})
			}).NativeNode()
		}(), note(func() string {
...
func OtpFieldDemo() *ui.Element {
	...
	return panel("OTP field", "Six slots: typing advances, paste distributes, and Backspace walks back. Completion is reported once per completed value.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return otpField61.Root().Children(func() *native.Node {
				var children []*native.Node
				for i := range 6 {
					if i == 3 {
						children = append(children, otpField61.SeparatorWith(ui.OtpFieldSeparatorProps{Index: ptr(i)}).Children("−").NativeNode())
					}
					s := inputStyle()
					s = s.Width(36)
					s = s.Height(40)
			...
			return ui.Fragment([]*native.Node{button("Fill 123456", func() {
				setCode("123456")
			}).Node, button("Clear", func() {
				setCode("")
			}).Node})
	...
	return panel("Date field", "Civil dates with no time zone. Up/Down steps a segment; typing advances to the next; min/max constrain the result.", func() *native.Node {
		...
		for _, field := range []struct {
			...
			children = append(children, func() *native.Node {
				...
				return dateField62.Root().Children(func() *native.Node {
					var children []*native.Node
...
func TimeFieldDemo() *ui.Element {
	...
	return panel("Time field", "12-hour and 24-hour civil times. A seconds segment exists only when ShowSeconds is enabled.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return timeField63.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{timeField63.SegmentWith(ui.TimeFieldSegmentProps{
					Segment:   "hour",
					PartProps: segment(40),
				}).NativeNode(), label(":").Node, timeField63.SegmentWith(ui.TimeFieldSegmentProps{
					Segment:   "minute",
					PartProps: segment(40),
				}).NativeNode(), timeField63.SegmentWith(ui.TimeFieldSegmentProps{
					Segment:   "period",
			...
			return timeField64.Root().Children(func() *native.Node {
				var children []*native.Node
...
func CalendarDemo() *ui.Element {
	...
	return panel("Calendar", "An application-declared grid with native keyboard navigation, one Tab stop, and reported month changes.", func() *native.Node {
		return ui.Fragment([]*native.Node{row(func() *native.Node {
			return ui.Fragment([]*native.Node{button("Previous month", func() {
				t, _ := time.Parse("2006-01", month())
				setMonth(t.AddDate(0, -1, 0).Format("2006-01"))
			}).Node, label(month).Node, button("Next month", func() {
				t, _ := time.Parse("2006-01", month())
				setMonth(t.AddDate(0, 1, 0).Format("2006-01"))
			}).Node})
			...
			return calendar65.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{ui.View().Child(func() *native.Node {
					var children []*native.Node
					...
					return ui.Fragment(children)
				}).Display("flex").Gap(3).Node, ui.For(
					func() [][]string {
							...
							for _, date := range week {
								...
								children = append(children, calendar65.DayWith(ui.CalendarDayProps{
									...
									PartProps: ui.PartProps{Style: func() ui.StyleBuilder {
										return ui.Style().Display("flex").AlignItems("center").JustifyContent("center").Width(32).Height(28).BorderRadius(7).BackgroundColor(choose(selected(), p().Accent, p().PanelAlt)).TextColor(choose(selected(), p().OnAccent, choose(strings.HasPrefix(date, month()), p().Ink, p().Faint))).FontSize(11).Hover(func(s ui.StyleBuilder) ui.StyleBuilder {
