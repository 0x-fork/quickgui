...
func FieldDemo() *ui.Element {
	...
	return panel("Field", "A control, its label, description, error, and validity. Validation triggers and debounce are reported by the core.", func() *native.Node {
		return ui.Fragment([]*native.Node{ui.Field.Root(
			ui.FieldRootProps{
				Required:               true,
				Invalid:                invalid,
			...
			func() *native.Node {
				return ui.Fragment([]*native.Node{ui.Field.Label(
					ui.FieldLabelProps{},
					"Email address",
				),
...
func FieldsetDemo() *ui.Element {
	...
	return panel("Fieldset", "A semantic group with a legend. A single flag disables every nested field.", func() *native.Node {
		...
		children = append(children, ui.Fieldset.Root(
			props,
			func() *native.Node {
				var children []*native.Node
				children = append(children, ui.Fieldset.Legend(ui.PartProps{}, "Profile"))
				children = append(children, ui.Fieldset.Description(
					ui.PartProps{},
				...
				for _, field := range []struct {
					...
					children = append(children, ui.Field.Root(
		...
		children = append(children, note(func() string { return "disabled " + strconv.FormatBool(saving()) + " · " + name() + " · " + org() }).Node)
		return ui.Fragment(children)
	})
}
func FormDemo() *ui.Element {
	...
	return panel("Form", "Validation appears on submission. Return in the email field also submits; Reset clears all form state.", func() *native.Node {
		return ui.Fragment([]*native.Node{ui.Fieldset.Root(
			columnPart(),
			func() *native.Node {
				return ui.Fragment([]*native.Node{ui.Fieldset.Legend(ui.PartProps{}, "Sign up"),
					ui.Field.Root(
						ui.FieldRootProps{
							Required:          true,
						...
						func() *native.Node {
							var children []*native.Node
							children = append(children, ui.Field.Label(
								ui.FieldLabelProps{},
								"Email",
							))
							control := fieldInput(email, setEmail, "you@example.com")
							control.OnSubmit = func(*native.Event) { submit() }
							children = append(children, ui.Field.Control(control))
							children = append(children, ui.Field.Error(
								ui.PartProps{Style: ui.Style().
									FontSize(12).
									TextColor(color(func(p palette) string { return p.Danger }))},
					...
					row(func() *native.Node {
						return ui.Fragment([]*native.Node{primary("Submit", submit).Node,
							button("Reset", func() {
								ui.Batch(func() {
									setEmail("")
									setPlan("pro")
			...
			note(func() string { return "attempts " + strconv.Itoa(attempts()) + " · " + result() }).Node})
	})
}
func InputDemo() *ui.Element {
	...
	return panel("Input", "Controlled native editors: single-line, password, and multiline. Return in the first field reports submission.", func() *native.Node {
		return ui.Fragment([]*native.Node{input(text, setText, "Type and press Return", ui.Style().Width(300), ui.OnSubmit(func(*native.Event) { setSubmits(submits() + 1) })).Node,
			input(secret, setSecret, "Password", ui.Style().Width(300), ui.Password(true)).Node,
			input(notes, setNotes, "Notes", ui.Style().Width(420).Height(96), ui.Multiline(true)).Node,
			note(func() string {
...
func numberControls() *native.Node {
	return ui.NumberField.Group(
		rowPart(),
		func() *native.Node {
			var children []*native.Node
			children = append(children, ui.NumberField.Decrement(control(), "−"))
			s := inputStyle()
			s = s.Width(110)
			children = append(children, ui.NumberField.Input(ui.NumberFieldInputProps{InputPartProps: ui.InputPartProps{PartProps: ui.PartProps{Style: s}}}))
			children = append(children, ui.NumberField.Increment(control(), "+"))
			return ui.Fragment(children)
		},
	)
}
func NumberFieldDemo() *ui.Element {
	...
	return panel("Number field", "Parsing, clamping, step snapping, and scrub gestures. Alt uses the small step; Shift uses the large step; Return commits.", func() *native.Node {
		return ui.Fragment([]*native.Node{ui.NumberField.Root(
			...
			func() *native.Node {
				...
				children = append(children, ui.NumberField.ScrubArea(
					ui.PartProps{Style: ui.Style().Cursor("ew-resize").Height(24)},
					"Drag here to change quantity",
				))
				children = append(children, numberControls())
				children = append(children, ui.NumberField.ScrubAreaCursor(
					ui.PartProps{},
					func() *native.Node {
						return ui.Show(
							state().Scrubbing,
							func() *ui.Element { return label("Scrubbing") },
						)
					},
				))
			...
			ui.NumberField.Root(
				...
				func() *native.Node {
					return ui.Fragment([]*native.Node{label("Read-only").Node, numberControls()})
				},
			),
			note(func() string {
...
func OtpFieldDemo() *ui.Element {
	...
	return panel("OTP field", "Six slots: typing advances, paste distributes, and Backspace walks back. Completion is reported once per completed value.", func() *native.Node {
		return ui.Fragment([]*native.Node{ui.OtpField.Root(
			...
			func() *native.Node {
				...
				for i := range 6 {
					if i == 3 {
						children = append(children, ui.OtpField.Separator(
							ui.OtpFieldSeparatorProps{Index: ptr(i)},
							"−",
						))
					}
					s := inputStyle()
					...
					s = s.FontSize(18)
					children = append(children, ui.OtpField.Input(ui.OtpFieldInputProps{
						Index:          i,
			...
			row(func() *native.Node {
				return ui.Fragment([]*native.Node{button("Fill 123456", func() { setCode("123456") }).Node, button("Clear", func() { setCode("") }).Node})
			}).Node,
			note(func() string { return "code " + code() + " · completed " + completed() }).Node})
	})
}
...
func DateFieldDemo() *ui.Element {
	...
	return panel("Date field", "Civil dates with no time zone. Up/Down steps a segment; typing advances to the next; min/max constrain the result.", func() *native.Node {
		...
		for _, field := range []struct {
			...
			children = append(children, ui.DateField.Root(
				...
				func() *native.Node {
					...
					for i, name := range field.segments {
						...
						children = append(children, ui.DateField.Segment(ui.DateFieldSegmentProps{
							Segment:   name,
		...
		children = append(children, note(func() string { return "US " + textValue(due()) + " · ISO " + textValue(iso()) }).Node)
		return ui.Fragment(children)
	})
}
func TimeFieldDemo() *ui.Element {
	...
	return panel("Time field", "12-hour and 24-hour civil times. A seconds segment exists only when ShowSeconds is enabled.", func() *native.Node {
		return ui.Fragment([]*native.Node{ui.TimeField.Root(
			...
			func() *native.Node {
				return ui.Fragment([]*native.Node{ui.TimeField.Segment(ui.TimeFieldSegmentProps{
					Segment:   "hour",
			...
			ui.TimeField.Root(
				...
				func() *native.Node {
					...
					for i, name := range []string{"hour", "minute", "second"} {
						...
						children = append(children, ui.TimeField.Segment(ui.TimeFieldSegmentProps{
							Segment:   name,
							PartProps: segment(40),
						}))
					}
			...
			note(func() string { return "12-hour " + textValue(at()) + " · precise " + textValue(precise()) }).Node})
	})
}
...
func CalendarDemo() *ui.Element {
	...
	return panel("Calendar", "An application-declared grid with native keyboard navigation, one Tab stop, and reported month changes.", func() *native.Node {
		return ui.Fragment([]*native.Node{row(func() *native.Node {
			return ui.Fragment([]*native.Node{button("Previous month", func() { t, _ := time.Parse("2006-01", month()); setMonth(t.AddDate(0, -1, 0).Format("2006-01")) }).Node,
				label(month).Node,
				button("Next month", func() { t, _ := time.Parse("2006-01", month()); setMonth(t.AddDate(0, 1, 0).Format("2006-01")) }).Node})
		}).Node,
			ui.Calendar.Root(
						...
						func() *native.Node {
							...
							for _, name := range []string{"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"} {
								children = append(children, ui.Text(
									name,
								).Width(32).TextAlign("center").FontSize(10).Node)
							}
							return ui.Fragment(children)
						},
					).Display("flex").Gap(3).Node,
						ui.For(
							func() [][]string { return monthGrid(month()) },
							func(week []string, index func() int) *native.Node {
								return ui.Calendar.Week(
									...
									func() *native.Node {
										...
										for _, date := range week {
											...
											children = append(children, ui.Calendar.Day(
												ui.CalendarDayProps{
													Day: date,
