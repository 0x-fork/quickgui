...
func AccordionDemo() *ui.Element {
	...
	return panel("Accordion", "Multiple open items, semantic headings, and lazy panels. Click a header or use the arrow keys.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return accordion7.Root().Children(func() *native.Node {
				...
				for index, entry := range entries {
					children_ = append(children_, accordion7.ItemWith(ui.AccordionItemProps{
						...
						PartProps: ui.PartProps{Style: func() ui.StyleBuilder {
							return ui.Style().Display("flex").FlexDirection("column").BorderRadius(10).BorderWidth(1).BorderColor(p().Border).BackgroundColor(p().PanelAlt).Overflow("hidden")
						}},
					}).Children(func() *native.Node {
						return ui.Fragment([]*native.Node{accordion7.Header(ui.PartProps{Style: ui.Style().Display("flex").Width("100%")}).Children(func() *native.Node {
							props := control()
							s := controlStyle()
							s = s.Width("100%")
							...
							s = s.Height(34)
							props.Style = s
							return accordion7.Trigger(props).Children(func() *native.Node {
								return ui.Fragment([]*native.Node{label(entry.title).Node, label(func() string {
									return choose(slices.Contains(open(), entry.value), "−", "+")
								}).Node})
							}).NativeNode()
						}).NativeNode(), accordion7.Panel(ui.PartProps{Style: ui.Style().Padding(12).PaddingTop(4)}).Children(func() *ui.Element {
...
func AlertDialogDemo() *ui.Element {
	...
	return panel("Alert dialog", "The backdrop refuses dismissal. Choose an answer or press Escape; focus returns to the trigger.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return alertDialog8.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{alertDialog8.Trigger(control()).Children("Delete branch…").NativeNode(), alertDialog8.Portal(overlay()).Children(func() *native.Node {
					return ui.Fragment([]*native.Node{alertDialog8.Backdrop(backdrop()).NativeNode(), alertDialog8.Popup(ui.DialogPopupProps{PartProps: popup(340)}).Children(func() *native.Node {
						return ui.Fragment([]*native.Node{alertDialog8.Title(ui.PartProps{}).Children(func() *ui.Element {
							return ui.Text("Delete “electron-parity”?").FontSize(15).FontWeight(700)
						}).NativeNode(), alertDialog8.Description(ui.PartProps{}).Children(func() *ui.Element {
							return muted("This gallery records the answer; it does not delete a real branch.")
						}).NativeNode(), row(func() *native.Node {
							return ui.Fragment([]*native.Node{alertDialog8.Close(control()).Children("Cancel").NativeNode(), button("Delete", func() {
								setOutcome("deleted")
								setOpen(false)
							}).Style(ui.Style().BackgroundColor(color(func(p palette) string {
								return p.Danger
...
func AvatarDemo() *ui.Element {
	...
	shell := func() ui.StyleBuilder {
		return ui.Style().Width(44).Height(44).BorderRadius(22).Display("flex").AlignItems("center").JustifyContent("center").BackgroundColor(p().Selection).BorderColor(p().Border).BorderWidth(1).Overflow("hidden")
	}
	return panel("Avatar", "Images and fallbacks share one accessible name. Failed loads report their actual status.", func() *native.Node {
		return ui.Fragment([]*native.Node{row(func() *native.Node {
			return ui.Fragment([]*native.Node{func() *native.Node {
				...
				return avatar9.Root().Children(func() *native.Node {
					return ui.Fragment([]*native.Node{avatar9.Image(ui.AvatarImageProps{
						Src:       "./avatar-does-not-exist.png",
						PartProps: ui.PartProps{Style: ui.Style().Width(44).Height(44)},
					}).NativeNode(), avatar9.Fallback(ui.AvatarFallbackProps{Delay: 120}).Children("AL").NativeNode()})
				}).NativeNode()
			}(), func() *native.Node {
				...
				return avatar10.Root().Children(func() *native.Node {
					return avatar10.Fallback(ui.AvatarFallbackProps{}).Children("GH").NativeNode()
				}).NativeNode()
			}()})
...
func ButtonDemo() *ui.Element {
	...
	return panel("Button", "Native press, focus, disabled state, and double-click handling.", func() *native.Node {
		return ui.Fragment([]*native.Node{row(func() *native.Node {
			return ui.Fragment([]*native.Node{primary("Primary", func() {
				setClicks(clicks() + 1)
				setLast("primary")
			}).Node, button("Secondary", func() {
				setClicks(clicks() + 1)
				setLast("secondary")
			}).Node, button("Disabled", func() {
				setLast("never")
			}).Disabled(true).Node, button("Double-click me", func() {
...
func CheckboxDemo() *ui.Element {
	...
	return panel("Checkbox", "Tri-state, read-only, and a parent whose mixed state is derived from ChildrenChecked.", func() *native.Node {
		...
		}, "Email me about releases"), checkbox(ui.CheckboxProps{
			DefaultChecked: true,
			ReadOnly:       true,
		}, "Read-only: focusable, refuses changes"), checkbox(ui.CheckboxProps{
			Parent:          true,
		...
		}, "Parent, derived from its children"), row(func() *native.Node {
			var children_ []*native.Node
...
func CheckboxGroupDemo() *ui.Element {
	...
	return panel("Checkbox group", "The parent toggles all values and derives its mixed state from the group.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return checkboxGroup11.Root().Children(func() *native.Node {
				var children_ []*native.Node
				children_ = append(children_, checkbox(ui.CheckboxProps{
					Parent: true,
					Checked: func() ui.CheckedState {
...
func CollapsibleDemo() *ui.Element {
	...
	return panel("Collapsible", "Closed panels are omitted; KeepMounted instead retains the panel with display:none.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return collapsible12.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{collapsible12.Trigger(control()).Children(func() *ui.Element {
					return label(func() string {
						return choose(open(), "Hide advanced options", "Show advanced options")
					})
				}).NativeNode(), collapsible12.Panel(ui.PartProps{}).Children(func() *ui.Element {
			...
			return collapsible13.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{collapsible13.Trigger(control()).Children("Toggle kept panel").NativeNode(), collapsible13.Panel(ui.PartProps{}).Children("Retained as display:none instead of omitted.").NativeNode()})
			}).NativeNode()
		}(), note(func() string {
...
func DialogDemo() *ui.Element {
	...
	return panel("Dialog", "An in-window dialog with a focus trap, a scrollable body, and a 120ms exit deadline.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return dialog14.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{dialog14.Trigger(control()).Children("Open dialog").NativeNode(), dialog14.Portal(overlay()).Children(func() *native.Node {
					return ui.Fragment([]*native.Node{dialog14.Backdrop(backdrop()).NativeNode(), dialog14.Popup(ui.DialogPopupProps{PartProps: popup(360)}).Children(func() *native.Node {
						return ui.Fragment([]*native.Node{dialog14.Title(ui.PartProps{}).Children(func() *ui.Element {
							return ui.Text("Publish this build?").FontSize(15).FontWeight(700)
						}).NativeNode(), dialog14.Description(ui.PartProps{}).Children(func() *ui.Element {
							return muted("Escape, the backdrop, and Cancel restore focus to the trigger.")
						}).NativeNode(), dialog14.Viewport(ui.PartProps{Style: ui.Style().Display("flex").FlexDirection("column").Gap(8).MaxHeight(160).Padding(4)}).Children(func() *ui.Element {
							return input(notes, setNotes, "Release notes").Multiline(true).Style(ui.Style().Width("100%").Height(64).PaddingTop(6))
						}).NativeNode(), row(func() *native.Node {
							return ui.Fragment([]*native.Node{dialog14.Close(control()).Children("Cancel").NativeNode(), primary("Publish", func() {
								setOpen(false)
							}).Node})
						}).Node})
					}).NativeNode()})
