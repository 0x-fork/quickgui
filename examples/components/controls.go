...
func gaugeTrack() ui.PartProps {
	return ui.PartProps{Style: func() ui.StyleBuilder {
		return ui.Style().Display("flex").Width(gaugeWidth).Height(8).BorderRadius(4).BackgroundColor(p().Track).Overflow("hidden")
	}}
}
func sliderParts(slider *ui.SliderComponent, values func() []float64) *native.Node {
	return slider.Control(ui.PartProps{Style: ui.Style().Position("relative").Display("flex").AlignItems("center").Width(gaugeWidth).Height(24)}).Child(func() *native.Node {
		var children []*native.Node
		children = append(children, slider.Track(ui.PartProps{Style: func() ui.StyleBuilder {
			return ui.Style().Display("flex").Width(gaugeWidth).Height(4).BorderRadius(2).BackgroundColor(p().Track)
			...
			style := func() ui.StyleBuilder {
				...
				return ui.Style().Height(4).BorderRadius(2).BackgroundColor(p().Accent).MarginLeft(left * gaugeWidth / 100).Width(strconv.FormatFloat(width, 'g', -1, 64) + "%")
			}
			if len(values()) > 1 {
		...
		for i := range values() {
			children = append(children, slider.Thumb(ui.SliderThumbProps{
				...
				PartProps: ui.PartProps{Style: func() ui.StyleBuilder {
					return ui.Style().Position("absolute").Left(values()[i]*gaugeWidth/100 - 10).Top(2).Width(20).Height(20).BorderRadius(10).BackgroundColor(p().Panel).BorderWidth(1).BorderColor(p().Border).BoxShadow("0 1px 2px #0000003d").FocusStyle(func(s ui.StyleBuilder) ui.StyleBuilder {
...
func MeterDemo() *ui.Element {
	...
	return panel("Meter", "A known range, with low/high/optimum thresholds and the core's formatted gauge state.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return meter35.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{row(func() *native.Node {
					return ui.Fragment([]*native.Node{meter35.Label(ui.PartProps{}).Children("Disk used").NativeNode(), meter35.Value(ui.PartProps{}).Children(func() *ui.Element {
						return label(func() string {
							return strconv.FormatFloat(level()[0], 'f', 0, 64) + "%"
						})
					}).NativeNode()})
				}).Node, meter35.Track(gaugeTrack()).Children(func() *native.Node {
					return meter35.Indicator(ui.PartProps{Style: func() ui.StyleBuilder {
						value := level()[0]
						return ui.Style().Height(8).BorderRadius(4).Width(strconv.FormatFloat(value, 'g', -1, 64) + "%").BackgroundColor(choose(value < 25, p().Danger, choose(value > 80, "#c88a00", p().Accent)))
					}}).NativeNode()
				}).NativeNode(), gaugeReadout().Node})
			}).NativeNode()
		}(), func() *native.Node {
			...
			return slider36.Root().Children(func() *native.Node {
	...
	return panel("Progress", "Determinate and indeterminate task progress. No timer keeps the gallery awake.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			progress37 := ui.NewProgress(ui.ProgressProps{
				...
				GaugeFormatProps: ui.GaugeFormatProps{Format: "fraction", PartProps: columnPart()},
			})
			return progress37.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{row(func() *native.Node {
					return ui.Fragment([]*native.Node{progress37.Label(ui.PartProps{}).Children("Uploading").NativeNode(), progress37.Value(ui.PartProps{}).Children(func() *ui.Element {
						return label(func() string {
							return choose(indeterminate(), "…", strconv.Itoa(done())+" / 12")
						})
					}).NativeNode()})
				}).Node, progress37.Track(gaugeTrack()).Children(func() *native.Node {
					return progress37.Indicator(ui.PartProps{Style: func() ui.StyleBuilder {
						return ui.Style().Height(8).BorderRadius(4).BackgroundColor(p().Accent).Width(strconv.FormatFloat(choose(indeterminate(), 35.0, float64(done())/12*100), 'g', -1, 64) + "%")
					}}).NativeNode()
				}).NativeNode(), gaugeReadout().Node})
			}).NativeNode()
		}(), row(func() *native.Node {
			return ui.Fragment([]*native.Node{button("−1", func() {
				setDone(max(0, done()-1))
			}).Node, button("+1", func() {
				setDone(min(12, done()+1))
			}).Node, button("Complete", func() {
				setDone(12)
			}).Node, button(func() string {
...
func RadioDemo() *ui.Element {
	...
	radio := func(value string, selected func() bool, caption string) *native.Node {
		...
		return radio38.Root().Children(func() *native.Node {
			return ui.Fragment([]*native.Node{radio38.Indicator(ui.PartProps{Style: func() ui.StyleBuilder {
				return ui.Style().Width(15).Height(15).BorderRadius(8).BorderWidth(choose(selected(), 4, 1)).BorderColor(choose(selected(), p().Accent, p().Border)).BackgroundColor(p().Control)
	...
	return panel("Radio", "Arrow keys select within a group; read-only groups retain their single Tab stop.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return radioGroup39.Root().Children(func() *native.Node {
			...
			return radioGroup40.Root().Children(func() *native.Node {
				var choices []*native.Node
...
func SeparatorDemo() *ui.Element {
	return panel("Separator", "Orientation, with application-defined thickness and colour. An adjustable divider is Splitter.", func() *native.Node {
		return ui.Fragment([]*native.Node{label("Above the rule").Node, func() *native.Node {
			separator41 := ui.NewSeparator(ui.SeparatorProps{
				Orientation: "horizontal",
				PartProps: ui.PartProps{Style: ui.Style().Height(1).BackgroundColor(color(func(p palette) string {
					return p.Border
				}))},
			})
			return separator41.Root().NativeNode()
		}(), label("Below the rule").Node, row(func() *native.Node {
			var children []*native.Node
				...
				if i > 0 {
					children = append(children, func() *native.Node {
						separator42 := ui.NewSeparator(ui.SeparatorProps{
							...
							PartProps: ui.PartProps{Style: ui.Style().Width(1).Height(18).BackgroundColor(color(func(p palette) string {
								return p.Border
							}))},
						})
...
func SliderDemo() *ui.Element {
	...
	return panel("Slider", "Clamping, step snapping, thumb ordering, and captured drag arithmetic belong to the core.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return slider43.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{slider43.Label(ui.PartProps{}).Children("Volume").NativeNode(), slider43.Value(ui.PartProps{}).Children(func() *ui.Element {
					state := ui.UseSliderState()
					return label(func() string {
						return textValue(state().DisplayValue) + " · dragging " + strconv.FormatBool(state().Dragging)
					})
				}).NativeNode(), sliderParts(slider43, volume)})
			}).NativeNode()
		}(), note(func() string {
			return fmt.Sprintf("volume %v · committed %s", volume(), committed())
		}).Node, func() *native.Node {
			...
			return slider44.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{slider44.Label(ui.PartProps{}).Children("Range · five steps between thumbs").NativeNode(), sliderParts(slider44, interval)})
			}).NativeNode()
		}(), note(func() string {
	...
	return panel("Splitter", "Drag or use arrow keys. The core conserves the total, enforces minima, and can collapse the last pane.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			splitter45 := ui.NewSplitter(ui.SplitterRootProps{
				...
				PartProps: ui.PartProps{Style: ui.Style().Display("flex").Height(140).BorderRadius(10).BorderWidth(1).BorderColor(color(func(p palette) string {
					return p.Border
				})).Overflow("hidden").BackgroundColor(color(func(p palette) string {
					return p.PanelAlt
				}))},
			})
			return splitter45.Root().Children(func() *native.Node {
				var children []*native.Node
				for i := range 3 {
					if i > 0 {
						children = append(children, splitter45.HandleWith(ui.SplitterPaneProps{
							Index: ptr(i - 1),
							PartProps: ui.PartProps{Style: ui.Style().Width(6).BackgroundColor(color(func(p palette) string {
								return p.Border
							})).Cursor("col-resize")},
						}).NativeNode())
					}
					children = append(children, splitter45.PaneWith(ui.SplitterPaneProps{
						Index:     ptr(i),
						PartProps: ui.PartProps{Style: ui.Style().Display("flex").AlignItems("center").JustifyContent("center")},
		...
		return ui.Fragment([]*native.Node{func() *native.Node {
			switch46 := ui.NewSwitch(ui.SwitchProps{
				...
				PartProps: ui.PartProps{
					AriaLabel: caption,
					Style: func() ui.StyleBuilder {
						return ui.Style().Width(44).Height(24).BorderRadius(12).Padding(2).Display("flex").AlignItems("center").BackgroundColor(choose(checked(), p().Accent, p().Track)).Transition("background-color 160ms ease-out")
					},
				},
			})
			return switch46.Root().Children(func() *native.Node {
				return switch46.Thumb(ui.PartProps{Style: func() ui.StyleBuilder {
					return ui.Style().Width(20).Height(20).BorderRadius(10).BackgroundColor("white").FlexShrink(0).Transform(choose(checked(), "translateX(20px)", "translateX(0px)")).Transition("transform 160ms ease-out")
				}}).NativeNode()
			}).NativeNode()
		}(), label(caption).Node})
	})
...
func SwitchDemo() *ui.Element {
	...
	return panel("Switch", "Animated thumb and track color, including a focusable read-only example.", func() *native.Node {
		return ui.Fragment([]*native.Node{switchControl(wifi, setWifi, false, "Wi-Fi").Node, switchControl(beta, setBeta, false, "Beta updates").Node, switchControl(managed, setManaged, true, "Managed by policy · read-only").Node, note(func() string {
...
func TabsDemo() *ui.Element {
	...
	return panel("Tabs", "Automatic activation, keyboard navigation, lazy panels, and measured indicator geometry.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return tabs47.Root().Children(func() *native.Node {
				var children []*native.Node
				children = append(children, tabs47.List(rowPart()).Children(func() *native.Node {
					var children []*native.Node
					for i, value := range []string{"overview", "usage", "limits"} {
						...
						}).Children(value).NativeNode())
					}
					children = append(children, tabs47.Indicator(ui.TabsIndicatorProps{
						Placement: "bottom",
						PartProps: ui.PartProps{Style: ui.Style().Height(2).BackgroundColor(color(func(p palette) string {
							return p.Accent
						}))},
					}).NativeNode())
					return ui.Fragment(children)
				...
				children = append(children, tabs47.PanelWith(ui.TabsPanelProps{Value: "overview"}).Children("An inactive panel contributes no layout or paint.").NativeNode())
				children = append(children, tabs47.PanelWith(ui.TabsPanelProps{Value: "usage"}).Children("Arrow keys move focus and select automatically.").NativeNode())
				children = append(children, tabs47.PanelWith(ui.TabsPanelProps{Value: "limits"}).Children("KeepMounted retains inactive panels with display:none.").NativeNode())
				return ui.Fragment(children)
			}).NativeNode()
		}(), note(func() string {
			return "active " + tab()
	...
	return panel("Toggle", "Buttons that stay pressed, with native toggle-button semantics.", func() *native.Node {
		return ui.Fragment([]*native.Node{row(func() *native.Node {
			return ui.Fragment([]*native.Node{func() *native.Node {
				...
				return toggle49.Root().Children(func() *native.Node {
					return ui.Fragment([]*native.Node{toggle49.Indicator(ui.PartProps{Style: ui.Style().Width(6).Height(6).BorderRadius(3).BackgroundColor(color(func(p palette) string {
						return p.Accent
					}))}).NativeNode(), label("Pinned").Node})
				}).NativeNode()
			}()})
	...
	return panel("Toggle group", "Single and multiple selection, roving focus, and a disabled item.", func() *native.Node {
		...
		for _, group := range []struct {
			...
			children = append(children, func() *native.Node {
				...
				return toggleGroup50.Root().Children(func() *native.Node {
					var children []*native.Node
					for _, value := range group.items {
...
func ToolbarDemo() *ui.Element {
	...
	return panel("Toolbar", "One roving Tab stop across items, a button, a link, and an input. Arrow keys skip the disabled item.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return toolbar51.Root().Children(func() *native.Node {
				var children []*native.Node
				children = append(children, toolbar51.Group(rowPart()).Children(func() *native.Node {
					var children []*native.Node
				...
				children = append(children, toolbar51.Separator(ui.PartProps{Style: ui.Style().Width(1).Height(20).BackgroundColor(color(func(p palette) string {
					return p.Border
				}))}).NativeNode())
				children = append(children, toolbar51.Button(ui.ToolbarItemProps{
					Value:     "share",
					PartProps: control(),
				}).Children("Share").NativeNode())
				children = append(children, toolbar51.Link(ui.ToolbarItemProps{
					Value:     "docs",
					PartProps: control(),
				}).Children("Docs").NativeNode())
				style := inputStyle()
