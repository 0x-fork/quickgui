...
func ScrollAreaDemo() *ui.Element {
	...
	return panel("Scroll area", "Forty rows in a fixed viewport. Scroll offsets, overflow flags, and thumb geometry come from the native scroll model.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			...
			return scrollArea31.Root().Children(func() *native.Node {
				return ui.Fragment([]*native.Node{scrollArea31.Viewport(ui.PartProps{Style: ui.Style().Width(320).Height(160).BackgroundColor(color(func(p palette) string {
					return p.PanelAlt
				})).BorderRadius(10).BorderWidth(1).BorderColor(color(func(p palette) string {
					return p.Border
				})).Overflow("hidden")}).Children(func() *native.Node {
					return scrollArea31.Content(ui.PartProps{Style: func() ui.StyleBuilder {
						return ui.Style().Display("flex").FlexDirection("column").PaddingLeft(10).Transform("translateY(" + strconv.FormatFloat(-state().Offset.Y, 'g', -1, 64) + "px)")
					}}).Children(func() *native.Node {
						var children_ []*native.Node
						for i := range 40 {
							children_ = append(children_, ui.Text("log line "+strconv.Itoa(i+1)).FontSize(12).Height(22).FlexShrink(0).LineHeight(22).TextColor(color(func(p palette) string {
								return p.Muted
							})).Node)
						}
						return ui.Fragment(children_)
					}).NativeNode()
				}).NativeNode(), scrollArea31.Scrollbar(ui.ScrollAreaScrollbarProps{
					Orientation: "vertical",
					PartProps: ui.PartProps{Style: ui.Style().Width(8).Height(160).BackgroundColor(color(func(p palette) string {
						return p.Track
					})).BorderRadius(4)},
				}).Children(func() *native.Node {
					...
					return scrollArea31.Thumb(ui.ScrollAreaThumbProps{
						...
						PartProps: ui.PartProps{Style: func() ui.StyleBuilder {
							return ui.Style().Width(8).BorderRadius(4).BackgroundColor(choose(live().Scrolling, p().Accent, p().Border))
						}},
					}).NativeNode()
				}).NativeNode()})
			}).NativeNode()
	...
	return panel("Table", "Five thousand sortable rows. Only the visible range is mounted. Drag column edges, select multiple rows, or double-click to activate.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			table32 := ui.NewTable(ui.TableRootProps{
				...
				PartProps: ui.PartProps{Style: ui.Style().Height(260).BorderRadius(10).BorderWidth(1).BorderColor(color(func(p palette) string {
					return p.Border
				})).BackgroundColor(color(func(p palette) string {
					return p.PanelAlt
				})).OverflowY("scroll")},
			})
			return table32.Root().Children(func() *native.Node {
				var children_ []*native.Node
				for _, column := range columns {
					children_ = append(children_, table32.Header(ui.TableHeaderProps{
						Column: column.ID,
						PartProps: ui.PartProps{Style: ui.Style().PaddingLeft(10).PaddingRight(10).FontSize(11).FontWeight(700).TextColor(color(func(p palette) string {
							return p.Faint
						}))},
				...
				children_ = append(children_, ui.KeyedFor(
					...
					func(entry func() visibleAsset, _ func() int) *native.Node {
						return table32.Row(ui.TableRowProps{Index: func() int {
							...
							return ui.Fragment([]*native.Node{table32.Cell(ui.TableCellProps{
								Column:    "name",
							...
							}).NativeNode(), table32.Cell(ui.TableCellProps{
								Column:    "size",
								PartProps: ui.PartProps{Style: ui.Style().PaddingRight(10).JustifyContent("flex-end")},
							}).Children(func() *ui.Element {
	...
	return panel("Tree", "Expand the pending element branch to request its children once. Arrow keys navigate, and double-click reports activation.", func() *native.Node {
		return ui.Fragment([]*native.Node{func() *native.Node {
			tree33 := ui.NewTree(ui.TreeRootProps{
				...
				PartProps: ui.PartProps{Style: ui.Style().Height(220).BorderRadius(10).BorderWidth(1).BorderColor(color(func(p palette) string {
					return p.Border
				})).BackgroundColor(color(func(p palette) string {
					return p.PanelAlt
				})).OverflowY("scroll")},
			})
			return tree33.Root().Children(func() *native.Node {
				return ui.KeyedFor(
					visible,
					...
					func(row func() flatTreeRow, _ func() int) *native.Node {
						return tree33.Row(ui.TreeRowProps{
							NodeID: row().Node.ID,
							PartProps: ui.PartProps{Style: func() ui.StyleBuilder {
								return ui.Style().Display("flex").AlignItems("center").Height(26).PaddingLeft(10 + row().Depth*16)
							}},
						}).Children(func() *ui.Element {
							return label(func() string {
