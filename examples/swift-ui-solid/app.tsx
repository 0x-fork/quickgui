...
const DEMOS: readonly DemoDefinition[] = [
  {
    ...
    label: "Button",
    description:
      "A native SwiftUI button with an SF Symbol, Liquid Glass styling, and an asynchronous press event delivered to Solid.",
  },
  {
    id: "slider",
    label: "Slider",
    description:
      "A controlled native slider with a bounded range and discrete steps. Its value is owned by the Solid signal below.",
  },
  {
    id: "toggle",
    label: "Toggle",
  ...
  },
  {
    ...
    label: "Stepper",
    description:
      "A bounded native stepper. SwiftUI performs the interaction and Solid receives the updated numeric value.",
  },
  {
  ...
  },
  {
    ...
    label: "Date Picker",
    description:
      "A native field-style date and time picker whose value crosses the bridge as a JavaScript Date.",
  },
  {
    id: "color-picker",
    label: "Color Picker",
  ...
  },
  {
    ...
    label: "Secure Field",
    description:
      "The secure variant uses SwiftUI's native concealed editor while keeping the same Solid value contract.",
  },
  {
    id: "popover",
    label: "Popover",
...
function Gallery() {
  ...
  const [interval, setInterval] = createSignal("week");
  const [scheduledAt, setScheduledAt] = createSignal(new Date());
  const [accent, setAccent] = createSignal("#3366ffff");
  const current = () => DEMOS.find((demo) => demo.id === page());

  function renderDemo(id: DemoId): unknown {
    switch (id) {
      case "button":
        return (
          <DemoPage
      ...
      case "date-picker":
        return (
          <DemoPage
            description={current()?.description ?? ""}
            state={() => scheduledAt().toLocaleString()}
          >
            <Host matchContents>
              <SwiftDatePicker
                label="Schedule"
      ...
      >
        ...
        >
          ...
          >
            ...
            <Text style={{ color: "#858b96", fontSize: 11 }}>Native SwiftUI · Solid state</Text>
          </View>
