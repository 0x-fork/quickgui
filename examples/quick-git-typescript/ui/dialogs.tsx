      ...
      <Dialog.Portal style={app.styles().dialogPortal}>
        ...
        >
          ...
          <Show when={props.description}>
            <Dialog.Description style={app.styles().dialogDescription}>
              {props.description}
            </Dialog.Description>
          </Show>
          <Dialog.Viewport
            style={{ display: "flex", flexDirection: "column", gap: 12, padding: 2 }}
          >
...
function NewBranchDialog() {
  ...
  const initialBase =
    (request?.kind === "new-branch" && request.from) || store.status()?.branch || "HEAD";
  ...
  const exists = createMemo(() =>
    store.refs().local.some((branch) => branch.name === name().trim()),
  );
  ...
  return (
    <DialogFrame
      ...
      actions={
        <PushButton
          ui={app}
          kind="primary"
          label={checkout() ? "Create and Switch" : "Create"}
          disabled={!valid()}
          onClick={() => void submit()}
        />
      }
    >
      <Field
        label="Name"
        error={problem() ?? (exists() ? "A branch with this name already exists." : undefined)}
      >
        <Input
          autoFocus
          value={name()}
...
function NewWorktreeDialog() {
  ...
  const availableBranches = createMemo(() =>
    store
      .refs()
      .local.filter((branch) => !branch.worktreePath)
  ...
  const effectivePath = () =>
    pathTouched() && path().trim()
      ? path().trim()
      : branchForPath()
        ? store.suggestWorktreePath(branchForPath())
        : "";
  const problem = createMemo(() =>
    mode() === "new" && name() ? branchNameProblem(name().trim()) : undefined,
  );
  const duplicate = createMemo(
    () => mode() === "new" && store.refs().local.some((branch) => branch.name === name().trim()),
  );
  const valid = () =>
    branchForPath().length > 0 && !problem() && !duplicate() && effectivePath().length > 0;
  ...
  async function submit(): Promise<void> {
    ...
    const added = await store.addWorktree(
      mode() === "new"
        ? { path: target, newBranch: name().trim(), base: base() }
        : { path: target, branch: existing() },
    );
    if (!added) return;
  ...
  return (
    ...
    >
      ...
      <Show when={mode() === "new"}>
        <Field
          label="Branch name"
          error={problem() ?? (duplicate() ? "A branch with this name already exists." : undefined)}
        >
          <Input
            autoFocus
            value={name()}
        ...
        >
          ...
          >
            <Text
              style={{
                fontSize: 13,
                color: existing() ? app.theme().text : app.theme().textTertiary,
              }}
            >
              {existing() || "Choose a branch"}
            </Text>
            <Icon name="chevron-down" size={12} color={app.theme().textTertiary} />
          </Select.Root>
        </Field>
      ...
      <Show when={agents().length > 0}>
        <Field
          ...
          hint={
            launch() === "none"
              ? undefined
              : "Opens Terminal in the new worktree and starts the agent there."
          }
        >
          <View style={{ display: "flex", flexDirection: "row", flexWrap: "wrap", gap: 6 }}>
            <ModeButton
              label="Just open it"
              active={launch() === "none"}
              onClick={() => setLaunch("none")}
            />
            {agents().map((agent) => (
              <ModeButton
                label={`Launch ${agent.label}`}
                active={launch() === agent.id}
                onClick={() => setLaunch(agent.id)}
              />
            ))}
          </View>
        </Field>
    ...
    >
      <Field label="Message (optional)">
        <Input
          autoFocus
          value={message()}
          placeholder="What were you in the middle of?"
          onInput={(event) => setMessage(event.value ?? "")}
          onSubmit={() => void submit()}
          style={app.styles().input}
        />
      </Field>
