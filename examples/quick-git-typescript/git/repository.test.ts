...
describe("Repository", () => {
  ...
  test("stages, unstages, previews untracked files, and commits", async () => {
    ...
    expect(
      unstagedChanges(status)
        .map((item) => item.path)
        .sort(),
    ).toEqual(["src/app.ts", "with space.md"]);
    ...
    expect(
      log[0]!.refs.some((ref) => ref.kind === "branch" && ref.name === "main" && ref.current),
    ).toBe(true);
  ...
  test("stages and unstages single hunks and lines through generated patches", async () => {
    ...
    const hunkWith = (candidate: typeof file, text: string) =>
      candidate.hunks.findIndex((hunk) =>
        hunk.lines.some((line) => line.kind === "added" && line.text === text),
      );
    ...
    expect(
      staged[0]!.hunks[0]!.lines.filter((line) => line.kind === "added").map((line) => line.text),
    ).toEqual(["tail added"]);
    ...
    const lineOneRemoved = hunk.lines.findIndex(
      (line) => line.kind === "removed" && line.text === "line 1",
    );
    const lineOneAdded = hunk.lines.findIndex(
      (line) => line.kind === "added" && line.text === "line one!",
    );
    expect(lineOneRemoved).toBeGreaterThanOrEqual(0);
    expect(lineOneAdded).toBeGreaterThanOrEqual(0);
    await repo.stagePatch(remaining, [
      { hunkIndex: topIndex, lines: new Set([lineOneRemoved, lineOneAdded]) },
    ]);
    staged = await repo.diffIndex("src/app.ts", undefined);
    const stagedText = staged[0]!.hunks.flatMap((hunk) =>
      hunk.lines
    ...
    const tailHunkIndex = staged[0]!.hunks.findIndex((hunk) =>
      hunk.lines.some((line) => line.text === "tail added"),
    );
    await repo.unstagePatch(staged[0]!, [{ hunkIndex: tailHunkIndex }]);
    staged = await repo.diffIndex("src/app.ts", undefined);
    expect(
      staged[0]!.hunks.flatMap((hunk) =>
        hunk.lines.filter((line) => line.kind === "added").map((line) => line.text),
      ),
    ).toEqual(["line one!"]);
    ...
    const xHunk = unstagedFiles[0]!.hunks.findIndex((hunk) =>
      hunk.lines.some((line) => line.kind === "added" && line.text === "x"),
    );
  ...
  test("manages branches, stashes, and worktrees", async () => {
    ...
    expect(refs.local.find((branch) => branch.name === "feature/login")?.worktreePath).toBe(
      worktrees[1]!.path,
    );
