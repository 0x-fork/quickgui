...
export class Repository {
  ...
  async createBranch(
    ...
    if (details.checkout ?? true)
      await this.runner.run(["switch", "-c", name, ...start], this.#options(options));
  ...
  async fetch(options: CommandOptions & { prune?: boolean } = {}): Promise<void> {
    await this.runner.run(
      ["fetch", ...((options.prune ?? true) ? ["--prune"] : []), "--no-write-fetch-head"],
      {
        ...this.#options(options),
        timeoutMs: NETWORK_GIT_TIMEOUT_MS,
      },
  ...
  async pull(options: CommandOptions = {}): Promise<void> {
    await this.runner.run(["pull", "--no-edit"], {
      ...this.#options(options),
      timeoutMs: NETWORK_GIT_TIMEOUT_MS,
    });
  }
  ...
  async push(
    ...
    if (details.setUpstream)
      args.push("--set-upstream", details.setUpstream.remote, details.setUpstream.branch);
...
/** Preserve literal tabs and newlines in NUL-delimited paths, including rename records. */
