...
describe("refs", () => {
  test("parses branches, remotes, tags, tracking, and worktree paths", () => {
    const output = [
      ...
      ["refs/tags/v1", "c".repeat(40), "ccccccc", "", "", "1500000000", " ", "Release", ""].join(
        "\0",
      ),
      "",
    ].join("\n");
    ...
    expect(parseStashes(output)).toEqual([
      {
        ...
        sha: "a".repeat(40),
        time: 1700000000,
        message: "WIP on main: abc1234 Fix things",
        branch: "main",
        summary: "abc1234 Fix things",
      },
      {
        ref: "stash@{1}",
        index: 1,
