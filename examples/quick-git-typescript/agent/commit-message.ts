...
export function buildPrompt(request: CommitMessageRequest): string {
  ...
  if (subjects.length > 0) {
    lines.push(
      "",
      "Recent commit subjects, newest first. Match their conventions (prefixes, tense, capitalization, punctuation):",
    );
  ...
  if (request.amending) {
    lines.push(
      "",
      "This commit amends the previous one, whose message was:",
      "",
      indent(request.amending),
    );
  }
  ...
  if (request.files.length > files.length)
    lines.push(`- … and ${request.files.length - files.length} more`);
  const { text, omitted } = boundDiff(request.diff, MAX_PROMPT_DIFF_CHARACTERS);
  lines.push("", "Diff:", "```diff", text.replace(/\n$/, ""), "```");
  if (omitted > 0)
    lines.push(
      "",
      `(${omitted} more changed ${omitted === 1 ? "file was" : "files were"} left out of the diff for length; the file list above is complete.)`,
    );
  return `${lines.join("\n")}\n`;
}
...
function summarizeFailure(agent: AvailableAgent, result: ProcessRun): string {
  ...
  return last
    ? `${agent.label}: ${last.slice(0, 200)}`
    : `${agent.label} exited with code ${result.exitCode}`;
}
