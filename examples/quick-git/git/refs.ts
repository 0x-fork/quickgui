...
export function parseRefs(output: string): RefCollections {
  ...
  for (const line of output.split("\n")) {
    ...
    if (fullName!.startsWith("refs/tags/")) {
      tags.push({ name: fullName!.slice(10), sha: sha!, shortSha: shortSha!, committerTime, subject: subject! });
      continue;
    }
...
export function branchNameProblem(name: string): string | undefined {
  ...
  if (/[\s~^:?*[\\\x00-\x1f\x7f]/.test(name)) return "Branch names cannot contain spaces or ~ ^ : ? * [ \\.";
  if (name.includes("..") || name.includes("@{")) return "Branch names cannot contain .. or @{.";
  if (name.startsWith("/") || name.endsWith("/") || name.includes("//")) return "Branch names cannot start or end with a slash.";
  if (name.endsWith(".") || name.endsWith(".lock") || name.split("/").some((part) => part.startsWith("."))) {
