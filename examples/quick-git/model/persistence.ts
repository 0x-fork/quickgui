...
export function rememberRepository(recent: readonly string[], path: string): string[] {
  ...
  return [target, ...recent.filter((entry) => canonicalPath(entry) !== target)].slice(0, MAX_RECENT_REPOSITORIES);
}
...
export function createPersistence(path: string | undefined, initial: PersistedState, saveDelay = 300): Persistence {
