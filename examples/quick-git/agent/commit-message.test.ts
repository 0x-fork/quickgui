...
const codex: AvailableAgent = { id: "codex", label: "Codex", command: "codex", description: "", executable: "/bin/codex" };
const claude: AvailableAgent = { id: "claude", label: "Claude", command: "claude", description: "", executable: "/bin/claude" };
...
describe("commit message parsing", () => {
  test("strips fences, labels, and quotes and splits subject from body", () => {
    ...
    expect(parseMessage("Commit message: \"Add worktree view\"")).toEqual({ subject: "Add worktree view", body: "" });
...
describe("running an agent", () => {
  test("invokes codex non-interactively with a read-only sandbox and reads its output file", async () => {
    ...
    const run: ProcessRunner = async (command, options) => {
      ...
      await writeFile(command[command.indexOf("-o") + 1]!, "feat: add commit generation\n\nUses the local agent.\n");
    ...
    expect(message).toMatchObject({ subject: "feat: add commit generation", body: "Uses the local agent.", agent: "codex" });
  ...
  test("invokes claude in print mode with tools disabled and reads stdout", async () => {
    const run: ProcessRunner = async () => ({ stdout: "Improve diff parsing\n", stderr: "", exitCode: 0 });
    const message = await generateCommitMessage(
      claude,
  ...
  test("reports failures with the last line the agent printed and honours cancellation", async () => {
    const failing: ProcessRunner = async () => ({ stdout: "", stderr: "error: not logged in\n", exitCode: 1 });
    const failure = await generateCommitMessage(claude, { repositoryName: "r", diff: "", files: [], recentSubjects: [] }, "/tmp", { run: failing }).catch(
      (error: unknown) => error,
    );
    ...
    const cancelled = await generateCommitMessage(claude, { repositoryName: "r", diff: "", files: [], recentSubjects: [] }, "/tmp", {
      run: cancelling,
