...
func Toolbar() *gui.Element {
	...
	return gui.View().Children(
		gui.Button().Child(toolbarIcon(branchIcon)).Style(app.Theme().Button("secondary")).Child(gui.Text(func() string {
			...
			return "…"
		}).MinWidth(0).LineClamp(1)).OnClick(func() {
		...
		gui.Show(
			...
			func() *gui.Element {
				...
				).AriaLabel("Upstream commit counts").Display("flex").AlignItems("center").Gap(3).FontSize(11).TextColor(app.Theme().TextSecondary).AppRegion("no-drag")
			},
		),
		gui.Show(
			store.Conflicts() > 0,
			func() *gui.Element {
				return gui.Text(strconv.Itoa(store.Conflicts()) + " conflicted").FontSize(11).FontWeight(600).TextColor(app.Theme().Warning).AppRegion("no-drag")
			},
		),
...
func toolbarBusy() *gui.Element {
	...
	return gui.View().Children(
		func() *native.Node {
			...
			return progress66.Root().Children(func() *native.Node {
				return progress66.Track(gui.PartProps{Style: gui.Style().Width("100%").Height(4).BorderRadius(2).BackgroundColor(app.Theme().BorderStrong)}).Children(func() *native.Node {
					return progress66.Indicator(gui.PartProps{Style: gui.Style().Width(12).Height(4).BorderRadius(2).BackgroundColor(app.Theme().TextSecondary)}).NativeNode()
				}).NativeNode()
			}).NativeNode()
		}(),
		...
		gui.Show(
			...
			func() *gui.Element {
				return gui.Button().Style(app.Theme().Button("secondary")).Child("Cancel").OnClick(store.CancelBusy)
			},
		),
...
func toolbarButton(label, icon string, click func(), disabled func() bool, iconOnly bool) *gui.Element {
	app := UseApp()
	return gui.Button().Style(app.Theme().Button("secondary")).Child(func() *native.Node {
	...
	}).AriaLabel(label).OnClick(click).Disabled(disabled).FlexDirection("row").Height(28).BorderRadius(7)
}
