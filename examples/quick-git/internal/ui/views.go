...
func ChangesView() *gui.Element {
	...
	return gui.View().Children(
		resizablePanel("Resize file list", store.ChangesSplit, store.SetChangesSplit, 220, 800, func() *native.Node {
			return gui.Fragment([]*native.Node{gui.Show(
				store.ChangeCount() > 0,
				func() *native.Node {
					return gui.Fragment([]*native.Node{fileList(model.ListUnstaged).Node, fileList(model.ListStaged).Node})
				},
				func() *gui.Element {
...
func fileList(list model.ListID) *gui.Element {
	...
	return gui.View().Children(
		gui.View().Children(
			...
			gui.Text(len(store.ListItems(list))).FontSize(11).TextColor(app.Theme().TextTertiary),
			gui.View().Flex(1),
			gui.Show(
				...
				func() *gui.Element {
					...
					return gui.Button().Style(app.Theme().Button("secondary")).Child(text).Disabled(store.Busy() != nil).OnClick(func() {
		...
		gui.Show(
			...
			func() *gui.Element {
				...
				return gui.Text(empty).PaddingLeft(12).PaddingBottom(10).FontSize(12).TextColor(app.Theme().TextTertiary)
			},
		),
...
func changeTable(list model.ListID) *native.Node {
	...
	table70 := gui.NewTable(gui.TableRootProps{
		...
		Columns: func() []gui.TableColumnDeclaration {
			return []gui.TableColumnDeclaration{{ID: "toggle", Track: "40px", Align: "center"}, {ID: "status", Track: "24px", Align: "center"}, {ID: "name", Track: "1fr", RowHeader: true}}
		},
	...
	return table70.Root().Children(func() *native.Node {
		return gui.KeyedFor(
			visible,
			...
			func(row func() visibleItem[git.ChangeItem], _ func() int) *native.Node {
				return table70.Row(gui.TableRowProps{
					...
					PartProps: gui.PartProps{Style: gui.Style().Hover(func(s gui.StyleBuilder) gui.StyleBuilder {
					...
					return gui.Fragment([]*native.Node{table70.Cell(gui.TableCellProps{
						Column:    "toggle",
					...
					}).NativeNode(), table70.Cell(gui.TableCellProps{
						Column:    "status",
						PartProps: gui.PartProps{},
					}).Children(func() *gui.Element {
						return gui.Text(string(row().Item.Code)).Width(16).FontWeight(700).TextColor(StatusColor(app.Theme(), string(row().Item.Code)))
					}).NativeNode(), table70.Cell(gui.TableCellProps{
						Column: "name",
						PartProps: gui.PartProps{
...
func stageToggle(list model.ListID, item func() git.ChangeItem) *native.Node {
	...
	return checkbox71.Root().Children(func() *native.Node {
		return checkbox71.Indicator(gui.PartProps{Style: checkboxBox(app.Theme(), checked)}).Children(func() *native.Node {
			return gui.Show(
				checked,
				func() *gui.Element {
...
func changeName(list model.ListID, item func() git.ChangeItem) *gui.Element {
	...
	return gui.View().Children(
		...
		gui.Show(
			counts() != "",
			func() *gui.Element {
				return gui.Text(counts).FontSize(11).FontFamily("monospace").TextColor(app.Theme().TextTertiary)
			},
		),
...
func commitComposer() *gui.Element {
	...
	store := app.Store
	return gui.View().Children(
		gui.Input().Style(app.Theme().InputStyle()).Placeholder("Commit summary").Value(store.Subject()).OnInputEvent(func(event *native.Event) {
			store.SetSubject(event.Value)
		}).OnSubmitEvent(func(*native.Event) {
			store.Commit()
		}),
		gui.TextArea().Placeholder("Description").Value(store.Body()).OnInputEvent(func(event *native.Event) {
		...
		gui.View().Children(
			...
			gui.Show(
				len(store.Agents()) > 0,
				func() *gui.Element {
					return gui.Button().Style(app.Theme().Button("secondary")).Child(func() string {
						if store.Generating() != nil {
							return "Generating…"
						}
						return "Generate"
					}).Disabled(store.Generating() != nil).OnClick(func() {
						store.GenerateMessage("")
					})
				},
			),
			gui.Button().Style(app.Theme().Button("primary")).Child("Commit").Disabled(!store.CanCommit()).OnClick(func() {
				store.Commit()
			}),
...
func DiffPane() *gui.Element {
	app := UseApp()
	store := app.Store
	return gui.View().Child(gui.Show(
		...
		func() *native.Node {
			return gui.Fragment([]*native.Node{gui.View().Children(
				...
				gui.Show(
					...
					func() *gui.Element {
						return gui.Text("+" + strconv.Itoa(store.DiffStats().Added)).FontSize(11).FontWeight(700).TextColor(app.Theme().Success).FontFamily("monospace")
					},
				),
				gui.Show(
					store.DiffStats().Removed > 0,
					func() *gui.Element {
						return gui.Text("-" + strconv.Itoa(store.DiffStats().Removed)).FontSize(11).FontWeight(700).TextColor(app.Theme().Danger).FontFamily("monospace")
					},
				),
				gui.Show(
					store.Diff().Loading && store.Diff().Diff == nil,
					func() *gui.Element {
						return gui.Text("Loading…").FontSize(11).TextColor(app.Theme().TextTertiary)
					},
				),
				diffActions(),
			).Display("flex").FlexDirection("row").AlignItems("center").Gap(8).Height(40).FlexShrink(0).PaddingLeft(14).PaddingRight(10).BorderBottomWidth(1).BorderColor(app.Theme().Border).Node, gui.Show(
				store.Diff().Error != "",
				func() *gui.Element {
					return gui.Text(store.Diff().Error).Padding(12).TextColor(app.Theme().Danger).FontSize(12)
				},
			), diffTable()})
		},
		func() *gui.Element {
		...
		func() *gui.Element {
			...
			return gui.View().Child(gui.Show(
				...
				func() *native.Node {
					...
					if mode == "staged" {
						children = append(children, gui.Button().Style(app.Theme().Button("primary")).Child("Unstage "+strconv.Itoa(lineCount)+" Lines").OnClick(func() {
							store.ApplySelectedLines("unstage")
						}).Node)
						return gui.Fragment(children)
					}
					children = append(children, gui.Button().Style(app.Theme().Button("danger")).Child("Discard Lines…").OnClick(func() {
						store.ApplySelectedLines("discard")
					}).Node)
					children = append(children, gui.Button().Style(app.Theme().Button("primary")).Child("Stage "+strconv.Itoa(lineCount)+" Lines").OnClick(func() {
				...
				func() *native.Node {
					var children []*native.Node
					if mode == "staged" {
						children = append(children, gui.Button().Style(app.Theme().Button("secondary")).Child("Unstage File").OnClick(func() {
					...
					children = append(children, gui.Button().Style(app.Theme().Button("danger")).Child("Discard…").OnClick(func() {
						if item := store.ActiveItem(); item != nil {
							confirmDiscard(app, []git.ChangeItem{*item})
						}
					}).Node)
					children = append(children, gui.Button().Style(app.Theme().Button("primary")).Child("Stage File").OnClick(func() {
						if item := store.ActiveItem(); item != nil {
...
func diffTable() *native.Node {
	...
	table72 := gui.NewTable(gui.TableRootProps{
		PartProps: gui.PartProps{Style: gui.Style().Flex(1).MinHeight(0).OverflowY("scroll").FontFamily("monospace").FontSize(MonoFontSize)},
		Columns: func() []gui.TableColumnDeclaration {
			return []gui.TableColumnDeclaration{{ID: "old", Track: "46px", Align: "end"}, {ID: "new", Track: "46px", Align: "end"}, {ID: "text", Track: "1fr"}}
		},
		RowCount: func() float64 {
	...
	return table72.Root().Children(func() *native.Node {
		return gui.KeyedFor(
			visible,
...
func diffTableRow(table *gui.TableComponent, row func() git.DiffRow, index func() int) *native.Node {
	...
	numberStyle := gui.Style().FontFamily("monospace").FontSize(MonoFontSize - 1).TextColor(theme.DiffLineNumber).TextAlign("right").PaddingRight(6).UserSelect("none")
	return table.Row(gui.TableRowProps{
		...
		PartProps: gui.PartProps{
			Group: true,
			Style: gui.Style().BackgroundColor(background).SelectedStyle(func(s gui.StyleBuilder) gui.StyleBuilder {
				return s.BackgroundColor(selected)
			}),
		},
	}).Child(func() *native.Node {
		return gui.Fragment([]*native.Node{table.Cell(gui.TableCellProps{
			Column:    "old",
		...
		}).NativeNode(), table.Cell(gui.TableCellProps{
			Column:    "new",
			PartProps: gui.PartProps{Style: gui.Style().BackgroundColor(gutter)},
		}).Child(func() *native.Node {
			return gui.Show(
			...
			return gui.View().Children(
				gui.View().Child(func() *native.Node {
					var children []*native.Node
					if mark != "" {
				...
				gui.Text(row().Text).Flex(1).MinWidth(0).FontFamily("monospace").FontSize(MonoFontSize).TextColor(textColor).WhiteSpace("nowrap"),
				gui.Show(
...
func historyTable() *native.Node {
	...
	table73 := gui.NewTable(gui.TableRootProps{
		PartProps: gui.PartProps{Style: gui.Style().Flex(1).MinHeight(0).OverflowY("scroll")},
		Columns: func() []gui.TableColumnDeclaration {
			return []gui.TableColumnDeclaration{{ID: "graph", Track: strconv.FormatFloat(graphWidth(), 'g', -1, 64) + "px"}, {ID: "subject", Track: "1fr", RowHeader: true}, {ID: "author", Track: "110px"}, {ID: "date", Track: "84px", Align: "end"}}
		},
		RowCount: func() float64 {
	...
	return table73.Root().Children(func() *native.Node {
		return gui.KeyedFor(
			visible,
...
func historyTableRow(table *gui.TableComponent, row func() visibleItem[git.Commit], graphWidth func() float64) *native.Node {
	...
	return table.Row(gui.TableRowProps{
		...
		PartProps: gui.PartProps{
			Style: gui.Style().Hover(func(s gui.StyleBuilder) gui.StyleBuilder {
				return s.BackgroundColor(app.Theme().Hover)
			}).SelectedStyle(func(s gui.StyleBuilder) gui.StyleBuilder {
				return s.BackgroundColor(app.Theme().Selection)
			}),
			OnContextMenu: func(*native.Event) {
				...
				native.PopupMenu(
					app.Window,
					[]native.MenuItem{{Label: "Copy SHA", Click: func() {
		...
		return gui.Fragment([]*native.Node{table.Cell(gui.TableCellProps{
			...
			return gui.View().Children(
				...
				gui.Text(row().Item.Subject).Flex(1).FontSize(12.5).LineClamp(1).MinWidth(0),
			).Display("flex").Flex(1).MinWidth(0).FlexDirection("row").AlignItems("center").Gap(6)
		}).NativeNode(), table.Cell(gui.TableCellProps{
			Column:    "author",
			PartProps: gui.PartProps{Style: gui.Style().MinWidth(0).PaddingRight(8).Overflow("hidden")},
		}).Child(func() *gui.Element {
			return gui.Text(row().Item.AuthorName).FontSize(11).TextColor(app.Theme().TextTertiary).LineClamp(1)
		}).NativeNode(), table.Cell(gui.TableCellProps{
			Column:    "date",
			PartProps: gui.PartProps{Style: gui.Style().PaddingRight(10)},
		}).Child(func() *gui.Element {
			return gui.Text(git.RelativeTime(row().Item.AuthorTime, time.Now())).FontSize(11).TextColor(app.Theme().TextTertiary).TextAlign("right")
		}).NativeNode()})
	}).NativeNode()
}
func HistoryView() *gui.Element {
	...
	return gui.View().Children(
		resizablePanel("Resize history", store.HistorySplit, store.SetHistorySplit, 260, 1000, func() *native.Node {
			...
			).Display("flex").FlexDirection("row").AlignItems("center").Height(32).FlexShrink(0).PaddingLeft(12).PaddingRight(8).Gap(8).BorderBottomWidth(1).BorderColor(app.Theme().Border).Node, gui.Show(
...
func commitFileTable() *native.Node {
	...
	table74 := gui.NewTable(gui.TableRootProps{
		PartProps: gui.PartProps{Style: gui.Style().Height(func() float64 {
			return float64(min(8, len(store.CommitDetail().Files)) * 24)
		}).FlexShrink(0).MinHeight(0).OverflowY("scroll")},
		Columns: func() []gui.TableColumnDeclaration {
			return []gui.TableColumnDeclaration{{ID: "status", Track: "36px", Align: "start"}, {ID: "name", Track: "1fr", RowHeader: true}}
		},
		RowCount: func() float64 {
	...
	return table74.Root().Children(func() *native.Node {
		return gui.KeyedFor(
			visible,
			...
			func(row func() visibleItem[git.CommitFile], _ func() int) *native.Node {
				return table74.Row(gui.TableRowProps{
					...
					return gui.Fragment([]*native.Node{table74.Cell(gui.TableCellProps{
						Column:    "status",
						PartProps: gui.PartProps{Style: gui.Style().PaddingLeft(12)},
					}).Children(func() *gui.Element {
						return gui.Text(row().Item.Status).Width(16).FontWeight(700).TextColor(StatusColor(app.Theme(), row().Item.Status))
					}).NativeNode(), table74.Cell(gui.TableCellProps{
						Column:    "name",
						PartProps: gui.PartProps{},
					}).Children(func() *gui.Element {
...
func commitDetail() *gui.Element {
	...
	return gui.View().Child(gui.Show(
		...
		func() *native.Node {
			...
			).Padding(12).Display("flex").FlexDirection("column").Gap(4).Node, gui.Show(
...
func BranchesView() *gui.Element {
	...
	return gui.View().Children(
		gui.View().Children(
			gui.Text("Local branches").FontSize(11).FontWeight(700).TextTransform("uppercase").TextColor(app.Theme().TextTertiary).Flex(1),
			gui.Button().Style(app.Theme().Button("primary")).Child("New Branch").OnClick(func() {
		...
		gui.For(
			...
			func(branch git.BranchRef, _ func() int) *gui.Element {
				return gui.Button().Style(rowStyle(app.Theme(), branch.Current)).Children(
					gui.Text(branch.Name).Flex(1).FontWeight(ternary(branch.Current, 700, 500)).LineClamp(1),
					gui.Show(
						branch.Current,
						func() *gui.Element {
					...
					gui.Text(branch.ShortSha).FontSize(11).TextColor(app.Theme().TextTertiary),
				).OnClick(func() {
				}).OnDoubleClick(func(*native.Event) {
					if !branch.Current {
					...
					native.PopupMenu(
						app.Window,
						[]native.MenuItem{{Label: "Switch to Branch", Click: func() {
							store.SwitchBranch(branch.Name)
						}}, {Label: "New Branch from Here…", Click: func() {
						...
						}}, {Type: "separator"}, {Label: "Delete Branch…", Click: func() {
		...
		gui.Show(
			...
			func() *gui.Element {
				return gui.Text("Remote branches").FontSize(11).FontWeight(700).TextTransform("uppercase").TextColor(app.Theme().TextTertiary).MarginTop(16)
			},
		),
		gui.For(
			func() []git.BranchRef {
			...
			func(branch git.BranchRef, _ func() int) *gui.Element {
				return gui.View().Children(
					gui.Text(branch.Name).Flex(1).LineClamp(1).TextColor(app.Theme().TextSecondary),
					gui.Text(branch.ShortSha).FontSize(11).TextColor(app.Theme().TextTertiary),
...
func WorktreesView() *gui.Element {
	...
	return gui.View().Children(
		gui.View().Children(
			gui.Text("Worktrees").FontSize(11).FontWeight(700).TextTransform("uppercase").TextColor(app.Theme().TextTertiary).Flex(1),
			gui.Button().Style(app.Theme().Button("primary")).Child("New Worktree").OnClick(func() {
		...
		gui.For(
			...
			func(worktree git.Worktree, _ func() int) *gui.Element {
				...
				return gui.Button().Style(rowStyle(app.Theme(), active)).Children(
					gui.Text(label).Flex(1).LineClamp(1),
					gui.Text(filepath.Base(worktree.Path)).FontSize(11).TextColor(app.Theme().TextTertiary),
				).OnClick(func() {
					store.SelectWorktree(worktree.Path)
				}).OnContextMenu(func(*native.Event) {
					items := []native.MenuItem{{Label: "Reveal in Finder", Click: func() {
						native.ShowItemInFolder(
							worktree.Path,
							func(error) {
							},
...
func StashesView() *gui.Element {
	...
	return gui.View().Children(
		gui.View().Children(
			gui.Text("Stashes").FontSize(11).FontWeight(700).TextTransform("uppercase").TextColor(app.Theme().TextTertiary).Flex(1),
			gui.Button().Style(app.Theme().Button("primary")).Child("Stash Changes").Disabled(store.ChangeCount() == 0).OnClick(func() {
			...
			func() *native.Node {
				return gui.For(
					...
					func(stash git.StashEntry, _ func() int) *gui.Element {
						return gui.Button().Style(rowStyle(app.Theme(), false)).Children(
							gui.View().FlexCol().Flex1().MinWidth(0).Children(
								gui.Text(stash.Summary).LineClamp(1),
								gui.Text(stash.Ref+" · "+git.RelativeTime(stash.Time, time.Now())).FontSize(11).TextColor(app.Theme().TextTertiary),
							),
							gui.Button().Style(app.Theme().Button("secondary")).Child("Apply").OnClick(func() { store.StashApply(stash.Ref) }),
							gui.Button().Style(app.Theme().Button("secondary")).Child("Pop").OnClick(func() { store.StashPop(stash.Ref) }),
						).OnContextMenu(func(*native.Event) {
							native.PopupMenu(
								app.Window,
								[]native.MenuItem{{Label: "Apply", Click: func() {
...
func emptyState(title, description string) *gui.Element {
	app := UseApp()
	return gui.View().Children(
		gui.Text(title).FontSize(15).FontWeight(500).TextColor(app.Theme().TextTertiary).TextAlign("center"),
		gui.Text(description).FontSize(12).LineHeight(17).TextColor(app.Theme().TextTertiary).TextAlign("center").MaxWidth(360),
