										...
										gui.ToastPartProps{
											...
											PartProps: gui.PartProps{
												Style: gui.Styles(
													...
													gui.BorderRadius(8),
													gui.Opacity(opacity),
													gui.Transform("translateX("+formatSwipe(entry.SwipeMovement)+"px)"),
												),
											},
										},
										func() {
...
func navRow(label string, selected func() bool, trailing any, onClick func()) {
	...
	gui.Button(
		func() {
			gui.Text(label, gui.Flex(1), gui.MinWidth(0), gui.FontSize(13), gui.LineClamp(1))
