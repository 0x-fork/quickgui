...
func App(store *model.Store, appearance reactive.Accessor[string], openRepository, openPath func(string), onMount func(AppContext)) gui.Component {
	return func() *native.Node {
		...
		return ProvideApp(context, func() *native.Node {
			...
			return toast76.Provider().Children(func() *gui.Element {
...
func shell(notifications *gui.ToastComponent) *gui.Element {
	...
	return gui.View().Children(
		gui.Show(
			...
			func() *native.Node {
				return gui.Fragment([]*native.Node{resizablePanel("Resize sidebar", store.SidebarWidth, store.SetSidebarWidth, 180, 420, func() *native.Node {
					return Sidebar()
				}, gui.Style().Display("flex").FlexDirection("column").Height("100%").MinWidth(0).MinHeight(0).FlexShrink(0).BackgroundColor(app.Theme().SidebarWash)), gui.View().Children(
					Toolbar(),
					mainView(),
					...
					return notifications.Positioner(gui.ToastPartProps{
						...
						return notifications.Root(gui.ToastPartProps{
							...
							return gui.Fragment([]*native.Node{gui.View().Width(3).AlignSelf("stretch").BorderRadius(2).BackgroundColor(color).Node, notifications.Content(gui.ToastPartProps{
								ToastID:   entry.ID,
								PartProps: gui.PartProps{Style: gui.Style().Display("flex").Flex(1).MinWidth(0).FlexDirection("column").Gap(2)},
							}).Child(func() *native.Node {
								return gui.Fragment([]*native.Node{notifications.Title(gui.ToastPartProps{
									ToastID:   entry.ID,
									...
									return gui.Text(title).FontSize(12.5).FontWeight(700).TextColor(app.Theme().Text).LineClamp(2)
								}).NativeNode(), gui.Show(
									func() bool {
										current := toast()
									...
									func() *native.Node {
										return notifications.Description(gui.ToastPartProps{
											...
											return gui.Text(description).FontSize(12).LineHeight(16).TextColor(app.Theme().TextSecondary).LineClamp(4)
										}).NativeNode()
									},
								)})
							}).NativeNode(), notifications.Close(gui.ToastPartProps{
								ToastID: entry.ID,
								PartProps: gui.PartProps{
...
func Welcome() *gui.Element {
	...
	return gui.View().Children(
		...
		gui.View().Children(
			gui.View().Child(icon(branchIcon, 32, func() string {
			...
			gui.Text("Open a repository to review changes, history, and worktrees.").FontSize(13).TextColor(app.Theme().TextSecondary).TextAlign("center").LineHeight(19),
			gui.Button().Style(app.Theme().Button("primary")).Child("Open Repository…").OnClick(func() {
				app.OpenRepository()
			}),
			gui.Show(
				...
				func() *gui.Element {
					return gui.View().Children(
						gui.Text("Recent").FontSize(11).FontWeight(700).LetterSpacing(0.4).TextTransform("uppercase").TextColor(app.Theme().TextTertiary).PaddingLeft(10).MarginBottom(4),
						gui.For(
							func() []string {
							...
							func(path string, _ func() int) *gui.Element {
								return gui.Button().Children(
									gui.View().Children(
										gui.Text(filepath.Base(path)).FontSize(13).FontWeight(600).TextColor(app.Theme().Text).LineClamp(1),
										gui.Text(shorten(filepath.Dir(path))).FontSize(11).TextColor(app.Theme().TextTertiary).LineClamp(1),
									).Display("flex").Flex(1).MinWidth(0).FlexDirection("column"),
									gui.Show(
										store.Opening() == path,
										func() *gui.Element {
											return gui.Text("Opening…").FontSize(11).TextColor(app.Theme().TextTertiary)
										},
									),
								).Disabled(store.Opening() != "").OnClick(func() {
									app.OpenRepositoryPath(path)
								}).Display("flex").FlexDirection("row").AlignItems("center").Gap(10).Height(40).PaddingLeft(10).PaddingRight(10).BorderRadius(8).BackgroundColor("transparent").Cursor("default").Hover(func(s gui.StyleBuilder) gui.StyleBuilder {
...
func Sidebar() *native.Node {
	...
	children = append(children, gui.View().Children(
		gui.Button().Children(
			gui.View().Child(icon(branchIcon, 18, func() string {
				return app.Theme().TextOnAccent
			...
			gui.View().Children(
				gui.Text(store.RepositoryName()).FontSize(13).FontWeight(700).TextColor(app.Theme().Text).LineClamp(1),
		...
		).AriaLabel("Repository actions").OnClick(func() {
			repositoryMenu(app)
		}).Display("flex").FlexDirection("row").AlignItems("center").Gap(9).Height(44).FlexShrink(0).PaddingLeft(8).PaddingRight(8).MarginBottom(6).BorderRadius(8).BackgroundColor("transparent").Cursor("default").Hover(func(s gui.StyleBuilder) gui.StyleBuilder {
			return s.BackgroundColor(app.Theme().Hover)
		}),
		gui.For(
			func() []struct {
...
func navRow(label string, selected func() bool, trailing any, onClick func()) *gui.Element {
	app := UseApp()
	return gui.Button().Style(rowStyle(app.Theme(), false)).Child(gui.Text(label).Flex(1).MinWidth(0).FontSize(13).LineClamp(1)).Child(navTrailing(trailing)).AriaLabel(label).When(
		selected(),
		gui.Selected(true),
	).OnClick(func() {
		onClick()
		...
		selected,
		gui.Style().BackgroundColor(func() string {
			return app.Theme().Selection
		}),
		gui.Style().Hover(func(s gui.StyleBuilder) gui.StyleBuilder {
...
func sectionRow(label string, count func() int, selected func() bool, onClick, action func()) *gui.Element {
	...
	return gui.View().Children(
		gui.Button().Children(
			gui.Text(label).Flex(1).MinWidth(0).FontSize(11).FontWeight(600).TextColor(color),
			gui.Show(
				count() > 0,
				func() *gui.Element {
					return gui.Text(count()).FontSize(11).FontWeight(600).TextColor(app.Theme().TextTertiary)
				},
			),
		).OnClick(func() {
			onClick()
		}).Display("flex").Flex(1).MinWidth(0).FlexDirection("row").AlignItems("center").Gap(6).Height(22).PaddingLeft(9).PaddingRight(6).BorderRadius(6).BackgroundColor("transparent").Cursor("default").Hover(func(s gui.StyleBuilder) gui.StyleBuilder {
			return s.BackgroundColor(app.Theme().Hover)
		}),
		gui.Button().Child(toolbarIcon(plusIcon)).Style(app.Theme().IconButton()).AriaLabel("Add "+strings.ToLower(label)).OnClick(func() {
			action()
		}),
	).Group(true).Display("flex").FlexDirection("row").AlignItems("center").Gap(4).MarginTop(14).PaddingRight(2)
}
