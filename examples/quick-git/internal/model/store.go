...
import (
	...
	"quickgui.example/quick-git/internal/agent"
...
func (s *Store) loadDiff(target *DiffTarget) {
	...
	if target == nil {
		s.dispatch(func() { s.setDiff(DiffState{}) })
		return
	}
...
func (s *Store) loadCommitDetail(sha string) {
	...
	s.mu.Unlock()
	if sha == "" || repo == nil {
		s.dispatch(func() { s.setCommitDetail(CommitDetailState{}) })
		return
	}
	ctx, cancel := context.WithCancel(context.Background())
	s.detailCancel = cancel
	s.dispatch(func() { s.setCommitDetail(CommitDetailState{SHA: sha, Loading: true}) })
	go func() {
...
func (s *Store) OpenRepository(path string) {
	...
	s.dispatch(func() { s.setOpening(target) })
	go func() {
...
func (s *Store) operation(label string, run func(*git.Repository, context.Context) error, refreshRefs bool, success *Notice) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	s.dispatch(func() { s.setBusy(&BusyState{Label: label, Cancel: cancel}) })
	go func() {
		err := run(repo, ctx)
		s.dispatch(func() {
			s.setBusy(nil)
...
func (s *Store) AddWorktree(path, newBranch, branch, base string) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	s.dispatch(func() { s.setBusy(&BusyState{Label: "Add worktree", Cancel: cancel}) })
	go func() {
...
func (s *Store) RemoveWorktree(path string, force bool) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	s.dispatch(func() { s.setBusy(&BusyState{Label: "Remove worktree", Cancel: cancel}) })
	go func() {
