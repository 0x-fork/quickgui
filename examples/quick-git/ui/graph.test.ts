...
describe("graph painting", () => {
  ...
  test("groups passing lanes and elbows by color and turns elbows the right way", () => {
    ...
    expect(byColor.get(2)).toContain(`M${laneX(0)} 13H${laneX(2) - radius}A${radius} ${radius} 0 0 1 ${laneX(2)} ${13 + radius}V26`);
    // Arriving from lane 3 on the right: down, a clockwise quarter turn, then level into the node.
    expect(byColor.get(3)).toContain(`M${laneX(3)} 0V${13 - radius}A${radius} ${radius} 0 0 1 ${laneX(3) - radius} 13H${laneX(0)}`);
  ...
  test("elbows toward the left turn counterclockwise", () => {
    ...
    const byColor = new Map(layers.map((layer) => [layer.color, layer.source]));
    const radius = LANE_WIDTH / 2;
    expect(byColor.get(0)).toContain(`M${laneX(2)} 13H${laneX(0) + radius}A${radius} ${radius} 0 0 0 ${laneX(0)} ${13 + radius}V26`);
    expect(byColor.get(1)).toContain(`M${laneX(1)} 0V${13 - radius}A${radius} ${radius} 0 0 0 ${laneX(1) + radius} 13H${laneX(2)}`);
  });
