...
export function paintGraphRow(row: GraphRow, width: number, height: number): GraphLayer[] {
  ...
  for (const join of row.joins) {
    ...
    add(join.color, `M${fromX} 0V${middle - radius}A${radius} ${radius} 0 0 ${sweep} ${fromX + direction * radius} ${middle}H${nodeX}`);
  }
  for (const edge of row.edges) {
    ...
    const sweep = direction > 0 ? 1 : 0;
    add(edge.color, `M${nodeX} ${middle}H${toX - direction * radius}A${radius} ${radius} 0 0 ${sweep} ${toX} ${middle + radius}V${height}`);
  }
  if (!segments.has(row.color)) segments.set(row.color, []);

  const layers: GraphLayer[] = [];
  for (const [color, paths] of segments) {
    const stroke = paths.length > 0 ? `<path d="${paths.join("")}" fill="none" stroke="#000" stroke-width="${LINE_WIDTH}"/>` : "";
    const node = color === row.color ? `<circle cx="${nodeX}" cy="${middle}" r="${NODE_RADIUS}" fill="#000"/>` : "";
    layers.push({
      color,
