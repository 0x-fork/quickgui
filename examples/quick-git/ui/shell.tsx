...
function Shell() {
  ...
  function handleSidebarPointer(event: QuickGuiEvent): void {
    ...
    if (pointer.phase === "move") {
      store.setSidebarWidth(Math.min(420, Math.max(180, sidebarDragStart + pointer.position.x - pointer.origin.x)));
    }
  }
    ...
    <View style={app.styles().app}>
      <Show when={store.repository()} fallback={<WelcomeView openRepository={app.openRepository} />}>
        ...
        <View
          ...
          hitSlopLeft={4}
          hitSlopRight={4}
          onPointer={handleSidebarPointer}
          style={{ width: 1, flexShrink: 0, cursor: "col-resize", appRegion: "no-drag", backgroundColor: app.theme().border }}
        />
...
function Notices() {
  ...
  const color = (type: string) =>
    type === "error" ? app.theme().danger : type === "success" ? app.theme().success : type === "warning" ? app.theme().warning : app.theme().accent;
  return (
    <Toast.Portal style={{ position: "absolute", right: 16, bottom: 16, width: 340, display: "flex" }}>
                ...
                >
                  ...
                  <Toast.Content toastId={entry.id} style={{ display: "flex", flex: 1, minWidth: 0, flexDirection: "column", gap: 2 }}>
                    <Toast.Title toastId={entry.id}>
                      <Text style={{ fontSize: 12.5, fontWeight: 700, color: app.theme().text, lineClamp: 2 }}>{toast()?.title ?? ""}</Text>
                    </Toast.Title>
                    <Show when={toast()?.description}>
                      <Toast.Description toastId={entry.id}>
                        <Text style={{ fontSize: 12, lineHeight: 16, color: app.theme().textSecondary, lineClamp: 4 }}>
                          {toast()?.description ?? ""}
                        </Text>
                      </Toast.Description>
                    </Show>
