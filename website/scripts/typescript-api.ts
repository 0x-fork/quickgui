...
for (const file of [
  ...
  function visit(node: any, namespace = "") {
    ...
    if (node.type === "TSTypeAliasDeclaration")
      aliases.set(prefix + namespace + name(node.id), node.typeAnnotation);
...
function properties(
  ...
  for (const field of node.body.body) {
    if (field.type !== "TSPropertySignature" && field.type !== "TSMethodSignature") continue;
