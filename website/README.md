...
- `bun run docs:api` reads Go declarations with `go/parser` and TypeScript declarations
  from the Solid bindings. Editorial descriptions live in
...
- `bun run docs:wasm` builds `crates/quickgui-docs-demo` with the production Rust
  ...
  `scripts/build-demo-sources.ts` with Go or TypeScript/JSX syntax highlighting.
