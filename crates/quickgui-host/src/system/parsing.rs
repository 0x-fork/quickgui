...
pub(crate) fn parse_permission_kind(value: &str) -> std::result::Result<PermissionKind, String> {
    match value {
        ...
        value => Err(format!("unknown native permission kind `{value}`")),
    }
}

pub(crate) fn permission_status_name(status: PermissionStatus) -> String {
    match status {
...
pub(crate) fn native_clipboard_item(item: ClipboardItem) -> NativeClipboardItem {
    let entries = item
        .entries()
        .iter()
...
pub(crate) fn parse_window_image_action(
    action: &str,
...
pub(crate) fn parse_global_shortcut_action(
    action: &str,
...
pub(crate) fn system_notification(
...
pub(crate) fn about_panel_options(
...
pub(crate) fn user_tasks(tasks: Vec<NativeUserTask>) -> std::result::Result<Vec<UserTask>, String> {
    tasks
        .into_iter()
        .map(|task| {
...
pub(crate) fn file_icon_size(value: &str) -> std::result::Result<FileIconSize, String> {
    match value {
...
pub(crate) fn parse_shell_action(
    action: &str,
    value: String,
