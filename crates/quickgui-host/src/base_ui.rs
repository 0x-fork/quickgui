...
impl NativeComponentStates {
    ...
    fn sync_avatar(
        ...
        if retained.source.as_ref() != source.as_ref() {
            ...
            } else if native_image_source(source.as_ref()).is_ok() && image_file_exists(&source) {
...
/// Whether a file-backed image source names an existing file; inline data URLs always do.
fn image_file_exists(source: &str) -> bool {
