    ...
    fn sync_slider(&mut self, key: u64, id: u32, node: &NativeNode, has_control: bool) {
        ...
        match self.sliders.get_mut(&key) {
            Some(retained) => {
                ...
                let reseed = declaration_changed && !acknowledged && !retained.state.is_dragging();
                if reseed || shape_changed {
                    ...
                    retained.state = configure(SliderState::range(minimum, maximum, &values));
                    retained.state.set_active_thumb(active);
                }
            }
            None => {
        ...
        for retained in self.number_fields.values_mut() {
            ...
            if retained.listens {
                ...
                enqueue_component_change(
                    ...
                    retained.owner,
                    serde_json::json!({
                        "numberValue": retained.state.value(),
                        "text": text.as_ref(),
        ...
        for retained in self.menubars.values_mut() {
            ...
            if retained.listens {
                enqueue_component_change(
                    ...
                    window,
                    retained.owner,
                    serde_json::json!({ "openIndex": current.0, "focusedIndex": current.1 }),
                );
            }
        }
                ...
                if commits {
                    enqueue_event(
                        ...
                        QueuedEvent {
                            ...
                            target: id,
                            value: Some(Arc::from(
                                serde_json::json!({ "numberValue": value })
                                    .to_string()
                                    .as_str(),
                            )),
                        },
