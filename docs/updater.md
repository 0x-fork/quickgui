...
`Options` can override the feed URL, current version, identifier, and public key. On macOS the public key must still match the embedded `SUPublicEDKey`. Development builds are disabled by default, including when using a release-built native library. Use `AllowDevelopment: true` only with a deliberately configured test feed and disposable app bundle.
...
Place the private key in `QUICKGUI_UPDATER_PRIVATE_KEY` (or `SPARKLE_PRIVATE_KEY`), using your CI secret store. Alternatively, `updates.ed25519SecretKey` points to a local key file. Sparkle-compatible 32-byte seeds and 64-byte seed/public key pairs are accepted.
