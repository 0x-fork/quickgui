...
the native host and terminal backend crates, and all three npm packages to match it. The job builds both macOS
native architectures for the core and terminal extension, runs the CLI tests and TypeScript 7 checks, verifies the npm
...
For each npm package (`@quickgui/native`, `@quickgui/native-terminal`, and `@quickgui/cli`), add an
...
It then publishes npm packages in the order `@quickgui/native`, `@quickgui/native-terminal`, and `@quickgui/cli`.
The terminal package is optional; the CLI resolves its exact version only when a Go import requires it.
