...
`quickgui init-extension <directory> --type go|rust|zig` scaffolds reusable components (Go) or an independent native service (Rust, Zig). Native providers export `quickgui_extension_v1` and do not depend on the renderer or a frontend runtime.

The same service artifact works from both frontends. Go opts in through an imported manifest and `host.RequireExtension`; TypeScript lists extension packages or directories in `native.extensions` in `quickgui.toml`. Paths resolve from the app project, and the CLI reads `quickgui.extension.json` inside each directory. Both package the exact provider release before application startup. All scaffold build scripts are TypeScript run with Bun.
