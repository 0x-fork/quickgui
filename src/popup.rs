...
/// Maximum nested menu-style native popup grabs retained by one application.
pub const MAX_GRABBING_POPUPS: usize = 32;

/// The point of a parent-space anchor rectangle used to position a popup.
...
/// The direction in which a popup extends from its anchor point.
#[derive(Clone, Copy, Debug, Default, Eq, Hash, PartialEq)]
...
bitflags! {
    /// Permitted screen-edge corrections for an anchored popup.
    #[derive(Clone, Copy, Debug, Default, Eq, Hash, PartialEq)]
...
/// Parent-relative placement and input behavior for an anchored native popup.
///
/// Coordinates use the parent's logical, top-left content coordinate space—the same space as
/// element bounds. The popup size comes from [`WindowOptions::size`](crate::WindowOptions::size).
...
pub struct PopupOptions {
    ...
    pub offset: Point,
    /// Menu-style popups take key focus and close on Escape or an outside mouse press.
    pub grab: bool,
    /// Whether pointer interaction may make the native popup the key window.
    ///
...
impl PopupOptions {
    ...
    pub const fn new(anchor_rect: Rect) -> Self {
        Self {
            ...
            constraint_adjustment: PopupConstraintAdjustment::FIT,
            offset: Point::ZERO,
            grab: true,
    ...
    pub const fn anchor(mut self, anchor: PopupAnchor) -> Self {
        self.anchor = anchor;
        self
    }

    pub const fn gravity(mut self, gravity: PopupGravity) -> Self {
        self.gravity = gravity;
        self
    }

    pub const fn constraint_adjustment(mut self, adjustment: PopupConstraintAdjustment) -> Self {
    ...
    /// Allow or forbid the popup from becoming the native key window after interaction.
    ///
    /// Forbidding key focus also disables menu-style grabbing because a grabbing popup must own
...
pub(crate) fn unconstrained_popup_rect(
    anchor_rect: Rect,
    size: Size,
...
pub(crate) fn place_popup(
    ...
    if options
        ...
        .contains(PopupConstraintAdjustment::FLIP_X)
    ...
    if options
        .constraint_adjustment
        .contains(PopupConstraintAdjustment::FLIP_Y)
    ...
    if options
        .constraint_adjustment
        .contains(PopupConstraintAdjustment::SLIDE_X)
    {
    ...
    if options
        .constraint_adjustment
        .contains(PopupConstraintAdjustment::SLIDE_Y)
    {
    ...
    if options
        .constraint_adjustment
        .contains(PopupConstraintAdjustment::RESIZE_X)
        && horizontal_overflow(result, visible_bounds) > 0.0
    {
    ...
    if options
        .constraint_adjustment
        .contains(PopupConstraintAdjustment::RESIZE_Y)
        && vertical_overflow(result, visible_bounds) > 0.0
    {
...
mod tests {
    ...
    fn default_menu_placement_grows_below_and_right() {
        let options = PopupOptions::new(Rect::new(40.0, 20.0, 100.0, 30.0));
        assert_eq!(
            place_popup(
                options.anchor_rect,
    ...
    fn vertical_flip_places_a_menu_above_an_edge_anchor() {
        let options = PopupOptions::new(Rect::new(40.0, 560.0, 100.0, 30.0));
        assert_eq!(
            place_popup(
                options.anchor_rect,
                Size::new(180.0, 120.0),
    ...
    fn horizontal_flip_preserves_the_near_edge_when_it_fits_better() {
        let options = PopupOptions::new(Rect::new(760.0, 40.0, 30.0, 30.0))
    ...
    fn slide_and_resize_are_independently_opt_in() {
        let options = PopupOptions::new(Rect::new(20.0, 20.0, 0.0, 0.0))
            .gravity(PopupGravity::BottomRight)
            .constraint_adjustment(
                PopupConstraintAdjustment::SLIDE_X | PopupConstraintAdjustment::RESIZE_Y,
            );
        assert_eq!(
            place_popup(
                options.anchor_rect,
    ...
    fn zero_sized_anchor_and_bounded_offset_are_valid() {
        assert!(
            PopupOptions::new(Rect::new(10.0, 20.0, 0.0, 0.0))
                .offset(4.0, -2.0)
                .is_valid(1_000.0, 1_000.0)
        );
        assert!(!PopupOptions::new(Rect::new(f32::NAN, 0.0, 0.0, 0.0)).is_valid(1_000.0, 1_000.0));
    }

    #[test]
    fn never_key_popup_is_interactive_without_a_grab() {
        let options = PopupOptions::new(Rect::ZERO).accepts_key_focus(false);
        assert!(!options.grab);
