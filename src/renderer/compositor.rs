...
struct GroupTextures {
    /// The group's own content texture; absent when the effect was dropped for a bound.
...
impl Compositor {
    ...
    pub(crate) fn render_scene(
        ...
        for group in groups.iter().rev() {
            textures.push(self.claim_textures(device, group, blur_scratch.as_ref(), frame));
        }
...
fn plan_group(
    ...
    let Some(content) = claimed.content.clone() else {
    ...
    if reads_destination && capture.is_none() {
        return dropped(inline_steps());
    }
    ...
    // The group's transform, expressed in physical pixels.
    ...
    let mask = physical_rect(group.bounds_box(), scale, frame);
    let matrix = effects.color_matrix;

    let mut blurs = Vec::new();
    if let (Some(blurred), Some(scratch)) = (&claimed.blurred, &claimed.scratch) {
