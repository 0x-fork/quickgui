...
describe("SwiftUI Solid bridge", () => {
  ...
  test("builds controlled native SwiftUI form controls and typed events", () => {
    ...
    expect(slider.properties.get(PropertyCode.SwiftUITestId)).toBe(
      "volume-slider",
    );
    ...
    expect(segmented.properties.get(PropertyCode.SwiftUIPickerStyle)).toBe(
      "segmented",
    );
    expect(segmentedTabs.properties.get(PropertyCode.SwiftUIPickerStyle)).toBe(
      "segmented",
    );
    ...
    expect(date.properties.get(PropertyCode.SwiftUIDatePickerComponents)).toBe(
      "dateAndTime",
    );
    ...
    expect(color.properties.get(PropertyCode.SwiftUIColorSupportsOpacity)).toBe(
      false,
    );
    expect(gauge.tag).toBe(NativeNodeTag.SwiftUIGauge);
    expect(gauge.properties.get(PropertyCode.SwiftUIGaugeStyle)).toBe(
      "accessoryLinearCapacity",
    );
    expect(
      gauge.properties.get(PropertyCode.SwiftUIGaugeMinimumValueLabel),
    ).toBe("0%");

    slider.listeners.get("input")!(
      new QuickGuiEvent("input", slider, "0.75"),
    );
    toggle.listeners.get("input")!(
      new QuickGuiEvent("input", toggle, "true"),
    );
    stepper.listeners.get("input")!(
      new QuickGuiEvent("input", stepper, "4"),
    );
    field.listeners.get("input")!(
      new QuickGuiEvent("input", field, "Grace"),
    );
    field.listeners.get("submit")!(new QuickGuiEvent("submit", field));
    picker.listeners.get("input")!(
      new QuickGuiEvent("input", picker, "list"),
    );
    segmented.listeners.get("input")!(
      new QuickGuiEvent("input", segmented, "week"),
    );
    date.listeners.get("input")!(
      new QuickGuiEvent("input", date, "1788528600.5"),
    );
    color.listeners.get("input")!(
      new QuickGuiEvent("input", color, "#ff000080"),
    );
