...
const { Menu, Shell, SpellChecker, Window, app } = await import("./index.ts");
await app.whenReady();
...
describe("application shell", () => {
  ...
  test("a rejected application service surfaces the native error", async () => {
    ...
    await expect(pending).rejects.toThrow(
      "this platform service is not implemented",
    );
  });
...
describe("native popup and per-window menus", () => {
  ...
  test("a popup position requires both coordinates and finite values", async () => {
    await expect(Menu.popup([{ label: "One" }], { window, x: 4 })).rejects.toThrow(
      TypeError,
    );
    await expect(
      Menu.popup([{ label: "One" }], { window, x: Number.NaN, y: 0 }),
